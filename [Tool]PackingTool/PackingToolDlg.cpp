...
BOOL CPackingToolDlg::OnInitDialog()
{
	...
	m_nFileNum = 0;
	m_nOldIndex = 0;
	m_sFileNum.Format( "%d File(s)", m_nFileNum );
...
//	m_BtnOpenBin.EnableWindow( FALSE );
	...
	// desc_hseos_주석기능_01
	// S 주석기능 추가 added by hseos 2007.05.10
	/* ..보류
	char szTitle[256];
	GetWindowText(szTitle, 256);
	sprintf(szTitle, "%s + 20070510(+Comment Func)", szTitle);
	SetWindowText( szTitle );
	*/
	// E 주석기능 추가 added by hseos 2007.05.10
...
void CPackingToolDlg::OnButtonOpen() 
{
	CFileDialog dlg( TRUE, NULL, "*.*", OFN_ALLOWMULTISELECT | OFN_HIDEREADONLY, 
					 "Text 파일(*.txt)|*.txt|Effect 파일(*.eff)|*.eff|모든 파일 (*.*)|*.*||", NULL );
	char buff[65535] = "";
...
void CPackingToolDlg::OnSelchangeListFile() 
{
	...
	// ..bin.txt 파일을 읽어서 그 내용을 Edit 박스에 설정한다.
	// ..위의 pFile->GetData() 에서 bin 내용을 설정하는데 그 내용에는 주석이 없기 때문.
	/* 보류
	char szTxt[2048];

	sprintf(szTxt, "%s.txt", m_sCurFile.GetString());
	FILE* fp = fopen(szTxt, "rt");
	if (fp)
	{
		char* pszTxtData = new char[MEGA];
		memset( pszTxtData, 0, MEGA );
		int nCnt = 0;
		int nSize = 0;
		while(TRUE)
		{
			// 파일의 길이를 모르니까 1바이트씩 읽기
			nSize = fread( &pszTxtData[nCnt], sizeof(char), 1, fp );
			if (nSize == 0) break;
			nCnt++;
		}

		m_Edit.SetWindowText(pszTxtData);
...
void CPackingToolDlg::SaveData( int index )
{
	...
	// E 파일끝의 공백문자 제거 추가 added by hseos 2007.05.22

	// desc_hseos_주석기능_01
	// S 주석기능 추가 added by hseos 2007.05.09
	// .."//" 키워드로 스크립트에 주석을 달 수 있도록 한다.
	// ..bin 파일로 저장할 때는 "//" 에 해당하는 라인을 제거한다.
	// ..대신 주석이 포함된 .bin.txt 파일을 저장한 후 .bin 파일을 로드할 때 .bin.txt 파일을 로드하도록 한다.
	/* 보류
	char szTxt[2048];

	sprintf(szTxt, "%s.txt", m_FileMng.GetFullFileName(index));
