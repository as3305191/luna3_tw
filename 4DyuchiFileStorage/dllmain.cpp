         ...
         LPVOID* ppv)
{
	...
	if (CLSID_4DyuchiFileStorage == rclsid)
	{
		hr = E_OUTOFMEMORY;
		pIFactory = new CF4DyuchiFileStorage;
		if (pIFactory)
		{
...
{
	HRESULT hr;
	I4DyuchiFileStorage*	pStorage = new CoStorage;
	if (!pStorage)
	{
		hr = E_OUTOFMEMORY;
		goto lb_return;
...
STDAPI DllRegisterServer(void)
{
 ...
 // DWORD		size = sizeof(TCHAR);
  ...
  StringFromGUID2(CLSID_4DyuchiFileStorage, wszID, GUID_SIZE);
  ...
  SetRegKeyValue(
    TEXT("CTS.CODE.1"),
    NULL,
    TEXT("DLL 3DEngine Component - by yuchi"));
  
  SetRegKeyValue(
    TEXT("CTS.CLSID_4DyuchiFileStorage.1"),
    TEXT("CLSID"),
    szID);
  ...
  SetRegKeyValue(
    TEXT("CTS.CLSID_4DyuchiFileStorage"),
    NULL,
   TEXT("CLSID"));
...
STDAPI DllUnregisterServer(void)
{
  ...
  RegDeleteKey(HKEY_CLASSES_ROOT, TEXT("CTS.CLSID_4DyuchiFileStorage\\CurVer"));
  RegDeleteKey(HKEY_CLASSES_ROOT, TEXT("CTS.CLSID_4DyuchiFileStorage\\CLSID"));
  RegDeleteKey(HKEY_CLASSES_ROOT, TEXT("CTS.CLSID_4DyuchiFileStorage"));

  RegDeleteKey(HKEY_CLASSES_ROOT, TEXT("CTS.CLSID_4DyuchiFileStorage.1\\CLSID"));
  RegDeleteKey(HKEY_CLASSES_ROOT, TEXT("CTS.CLSID_4DyuchiFileStorage.1"));
