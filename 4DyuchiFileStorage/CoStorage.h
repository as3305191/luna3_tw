...
public:
	...
	void*	__stdcall	MapPackFile(char* szPackFileName, DWORD dwSecretKey, BOOL bFileShareMode=FALSE);
	void	__stdcall	UnmapPackFile(void* pPackFileHandle);
	BOOL	__stdcall	Initialize(DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen, DWORD dwSecretKey,FILE_ACCESS_METHOD accessMethod);
