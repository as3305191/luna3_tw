...
m_bSelectMode(FALSE),
m_bPickColorMode(FALSE),
m_bDrawSelectedRect(FALSE),
m_dwSelectedVertexNum(0),
m_pSelectedVertexIndex(0),
m_dwMaxSelectedVertexNum(0),
m_dwSelectedTileNum(0),
m_pSelectedTileIndex(0),
m_dwMaxSelectedTileNum(0),
m_dwCurrentViewportIndex(0),
m_dwArrowRefIndex(0),
m_dwMoveAxis(0),
m_pBrush(0),
m_pHash(0),
...
m_gxlSelected(0),
m_gxtSelected(0),
m_Mouse(new MToolMouseStatus),
m_pEditWindow(0),
m_pExecutive(0),
m_pRendererForPalette(0),
m_pFileStorage(0),
m_pFont(0),
m_dwHFieldDetail(0),
m_dwEditMode(0),
...
m_pPalette(0),
m_pTileSetDialog(0),
m_pEditTileSetDialog(0),
m_pEditEntryDialog(0),
m_pLoadFailedTextureDialog(0),
m_pEditZoneDialog(0),
m_bDisplayCameraInfo(TRUE),
m_dwRenderToolMode(FALSE),
m_bRenderStaticModel(TRUE),
m_dwStaticModelAlpha(255),
m_dwHFieldAlpha(255),
m_dwViewHFieldAlpha(255),
m_bIsHFieldCreateMode(FALSE),
...
m_bLightCopy(FALSE),
m_bObjectCopy(FALSE),
m_CurrentViewMode(VIEW_TYPE_TOP),
mHistoryManager(new CHistoryManager)
{
	ZeroMemory(&m_v3PickedPos, sizeof(m_v3PickedPos));
	ZeroMemory(&m_planePicked, sizeof(m_planePicked));
	ZeroMemory(&m_v2PointSelectRect, sizeof(m_v2PointSelectRect));
	ZeroMemory(&m_v3ArrowPos, sizeof(m_v3ArrowPos));
	ZeroMemory(&m_hfDesc, sizeof(m_hfDesc));
	ZeroMemory(&m_hfcreate, sizeof(m_hfcreate));
	ZeroMemory(&m_tileInfo, sizeof(m_tileInfo));
	ZeroMemory(&m_aLoadFailedTextureTable, sizeof(m_aLoadFailedTextureTable));
	ZeroMemory(&m_WorldBox, sizeof(m_WorldBox));
	ZeroMemory(&m_v3Clicked, sizeof(m_v3Clicked));
...
CTool::~CTool()
{
	...
	if (m_pEditZoneDialog)
	{
		delete m_pEditZoneDialog;
		m_pEditZoneDialog = NULL;
	}
...
{	
	...
	{
		::MessageBox(NULL,"Ÿ�� �ȷ�Ʈ�� ���� �������ּ���","Error",MB_OK);
		return FALSE;
	}
...
void CTool::CleanZoneData()
{
	for (DWORD i=0; i<MAX_ZONE_NUM; ++i)
	{
		m_gxZoneData[i].dwMoveChar = 1;
		m_gxZoneData[i].dwMoveMon = 1;
	}
}
...
BOOL CTool::Initialize(CWnd* pWnd)
{
	...
	HMODULE executiveHanldeModule = LoadLibrary("Executive.dll");
	CREATE_INSTANCE_FUNC pFunc = (CREATE_INSTANCE_FUNC)GetProcAddress(
		executiveHanldeModule,
		"DllCreateInstance");
	HRESULT handleResult = pFunc((LPVOID*)&m_pExecutive);

	if(S_OK != handleResult)
	{
		MessageBox(
			0,
			_T("Executive.dll is not found"),
			_T(""),
			MB_OK);
		return FALSE;
	}
	...
	const BOOL isLoadedFileStorage = m_pExecutive->InitializeFileStorageWithoutRegistry(
		_T("FileStorage.dll"),
		50000,
		10000,
		MAX_PATH,
	...
	if(FALSE == isLoadedFileStorage)
	{
		return FALSE;
	}

	m_pExecutive->GetFileStorage(&m_pFileStorage);
	const BOOL isLoadedRender = m_pExecutive->InitializeWithoutRegistry(
		"Geometry.dll",
		"Renderer.dll",
		pWnd->m_hWnd,
		0, 
	...
	m_pEditZoneDialog = new CDialogEditZone;
	m_pEditZoneDialog->DisableView();
	///
	...
	m_Mouse->Initialize(m_pExecutive);
	...
	SetRenderToolMode(RENDER_MODE_DEFAULT|RENDER_MODE_TOOL);
	...
	m_szCurrentMapFileName = DEFAULT_SS3D_MAP_FILE_NAME;
	m_szCurrentTilePaletteFileName = _T("");
	m_szCurrentTileSetFileName = _T("");
...
void CTool::CreateBrushArea()
{
	...
	m_gxhLightPrj = m_pExecutive->CreateGXLight(&imagelight,NULL,NULL,
		0x00,		// ADD�� ����Ʈ�� �Ѹ���.�ؽ��Ĵ� ������ ��ġ�� ������ ������ �������ȴ�.0x00�� ��� ���ϱⰡ �ȴ�.�� �� ��ο�����.
		&mtl,		// ������ �ؽ��ĸ� �����ϴ� ���͸��� ��ũ���� 
	//	GXLIGHT_TYPE_ENABLE_SHADOW |
		GXLIGHT_TYPE_ENABLE_IMAGE_PROJECTION | // �ؽ��� �̹����� ���������ϴ� ����Ʈ���� ����.
	...
	v3Pos.z		=	0.0f;
 	...
 	m_pExecutive->GXLSetPosition( m_gxhLightPrj, &v3Pos);		// �ʱ� ��ġ�� ����.
...
BOOL CTool::ExportHeightField(char* szFileName)
{
	if (!m_pHField)
	{
		MessageBox(NULL,"������ ����Ʈ�ʵ尡 �����ϴ�.","Error",MB_OK);
		return FALSE;
	}
...
BOOL CTool::ExportTileTable(char* szFileName)
{
	...
	// ���� ����...
	fwrite(&hfDesc.dwTileNumX ,sizeof(DWORD),1,fp);	// x���� ����.
	fwrite(&hfDesc.dwTileNumZ ,sizeof(DWORD),1,fp);	// z���� ����.
	fwrite(hfDesc.pwTileTable,sizeof(WORD),hfDesc.dwTileNumX*hfDesc.dwTileNumZ,fp);	// Ÿ�� ���� ����.
	
	m_dwZoneNum = MAX_ZONE_NUM;
	fwrite(&m_dwZoneNum, sizeof(DWORD),1,fp);
	fwrite(m_gxZoneData, sizeof(GXZONE_DESC),MAX_ZONE_NUM,fp);
...
BOOL CTool::ImportHeightField(char* szFileName,DWORD dwIndexBufferNum)
{
	...
	if (!m_pHField->ReadFile(szFileName,dwIndexBufferNum,NULL,HFIELD_MODEL_LOAD_ENABLE_DRAW_ALPHAMAP | HFIELD_MODEL_LOAD_ENABLE_SHADE))
	{
		m_pHField->Release();
		m_pHField = NULL;
		m_szHFieldName.Empty();
		return FALSE;
	}

	m_szHFieldName = szFileName;
...
BOOL CTool::ImportViewOnlyHeightField(char* szFileName)
{
	...
	BOOL bResult = m_pExecutive->GetGeometry()->CreateHeightField(&m_pHFieldViewOnly,0);
...
{
	ImportHeightField(
		LPTSTR(m_szHFieldName.GetString()),
		dwNum);
}
...
void CTool::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_bSelectMode)
	{
		if( nFlags & MK_LBUTTON)
		{
			...
			m_v2PointSelectRect[0].x = (float)m_ptMousePrvDown.x;
			m_v2PointSelectRect[0].y = (float)m_ptMousePrvDown.y;

			m_v2PointSelectRect[1].x = (float)m_ptMousePrvDown.x;
			m_v2PointSelectRect[1].y = (float)point.y;

			m_v2PointSelectRect[2].x = (float)point.x;
			m_v2PointSelectRect[2].y = (float)point.y;

			m_v2PointSelectRect[3].x = (float)point.x;
			m_v2PointSelectRect[3].y = (float)m_ptMousePrvDown.y;
	...
	BOOL		bDirty;
	bDirty = FALSE;
	if(m_Mouse->MoveMouse( nFlags, point.x, point.y, *(DWORD*)&m_CurrentViewMode))
	{
		Render();
		goto lb_return;
	}
...
void CTool::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'W':
		{
			m_Mouse->MoveCamera_Forward( moveSpeed );
			Render();
		}
		break;
	case 's':
	case 'S':
		{
			m_Mouse->MoveCamera_Backward( moveSpeed );
			Render();
		}
		break;
	case 'a':
	case 'A':
		{
			m_Mouse->MoveCamera_LeftSideStep( moveSpeed );
			Render();
		}
		break;
	case 'd':
	case 'D':
		{
			m_Mouse->MoveCamera_RightSideStep( moveSpeed );
			Render();
		}
		break;
	case 'x':
	case 'X':
		{
			m_Mouse->MoveCamera_Up( moveSpeed );
			Render();
		}
		break;
	case 'c':
	case 'C':
		{
			m_Mouse->MoveCamera_Down( moveSpeed );
			Render();
		}
		break;
	}
	...
	if (m_pHField)
	{
		...
		else
		{
			...
			if (m_pHField->GetTilePosWithScreenCoord(&m_v3Clicked,&fDist,&dwPosX,&dwPosY,pt,m_dwCurrentViewportIndex))
			{
				...
				{
					CPutTileHistory& history = mHistoryManager->Allocate<CPutTileHistory>();
					history.SetValue(
						CPoint(dwPosX, dwPosY),
						lastTileIndex,
						m_pPalette->GetCurrentTileIndex());
					Render();
				}
			}
		}
	...
	if (m_pHField)
	{
		...
		if (m_pHField->GetTilePosWithScreenCoord(&m_v3Clicked,&fDist,&dwPosX,&dwPosZ,pt,m_dwCurrentViewportIndex))
		{
			...
			for(z=0;z<3;++z)
			{
				...
				{
					ix = dwPosX + x - 1;
					m_pHField->SetTile(ix,iz,res[z*3+x].GetTileIndex());
				}
			}			
		}
...
//	m_pExecutive->DeleteAllGXDecal();
	...
	CleanZoneData();
	m_pEditZoneDialog->SetZoneDesc(m_gxZoneData);
	////
...
BOOL CTool::OnClickHFieldVertexColor(POINT* pt,VECTOR3* pv3IntersectPoint,UINT nFlags)
{
	...
	DWORD dwColor = g_pDlgBarHField->GetLightTexelColor();
...
//		DWORD	dwCount = m_pExecutive->GXOGetMultipleObjectWithScreenCoord(pickDesc,32,pt,m_dwCurrentViewportIndex,PICK_TYPE_PER_BONE_OBJECT);
	//	m_gxoSelected = m_pExecutive->GXOGetObjectWithScreenCoord(&m_v3PickedPos,&dwModelIndex,&dwObjIndex,&fDist,pt,m_dwCurrentViewportIndex,PICK_TYPE_PER_BONE_OBJECT);
		...
		if (m_gxoSelected)
		{
			...
			if ( m_dwEditMode & (EDIT_TYPE_MOVE | EDIT_TYPE_ROT | EDIT_TYPE_SCALE))
			{
				GXOBJECT_PROPERTY property = {0};
				property.dwID = m_pExecutive->GetID(
					m_gxoSelected);
				property.bApplyHField = m_pExecutive->GXOIsHFieldApply(
					m_gxoSelected);
				property.bAsEffect = m_pExecutive->GXOIsAsEffect(
					m_gxoSelected);
				property.bLock = m_pExecutive->IsLockTransform(
					m_gxoSelected);
				m_pExecutive->GXOGetDirection(
					m_gxoSelected,
					&property.v3Axis,
					&property.fRad);
				m_pExecutive->GXOGetPosition(
					m_gxoSelected,
					&property.v3Pos);
				m_pExecutive->GXOGetScale(
					m_gxoSelected,
					&property.v3Scale);

				CMoveObjectHistory& history = mHistoryManager->Allocate< CMoveObjectHistory >();
				history.SetValue(
					property,
					m_gxoSelected);
			}
...
void CTool::OnRClickGXObject(POINT* ptCursor,UINT nFlags)
{
	...
	if (gxo)
	{
		...
		gxoproperty.dwID = m_pExecutive->GetID(gxo);					// ���̵� �ʱ�ȭ..
		...
		strcpy(gxoproperty.name, (char*)m_pExecutive->GetData(gxo));
...
void CTool::OnRClickGXTrigger(POINT* ptCursor,UINT nFlags)
{
	...
	if (gxt)
	{
		...
		gxtproperty.dwID = m_pExecutive->GetID(gxt);					// ���̵� �ʱ�ȭ..
...
BOOL CTool::ViewEditZone()
{
	if (!m_bViewEditZone)
	{
		m_bViewEditZone = TRUE;
		m_pEditZoneDialog->EnableView();
	}
	else
	{
		m_bViewEditZone = FALSE;
...
void CTool::OnLButtonDown(UINT nFlags, CPoint* point) 
{
	...
	case EDIT_MODE_TILE:
		//In
		{
			...
			*/
			...
			if(m_pSectionSplitter->bSectionInputMode)
				...
				OnClickTile(&ptCursor,nFlags);		/// Ÿ�� Ŭ��������. ��� ����̴�.
			//--
		...
		break;
	case EDIT_MODE_DRAW_LIGHTMAP:
		OnClickLightMap(&ptCursor,nFlags);			/// ����Ʈ�� �׸� ��?
		break;
	case EDIT_MODE_DRAW_VERTEXCOLOR:
		{
			BOOL	bResult;
...
void CTool::OnRClickLight(POINT* ptCursor,UINT nFlags)
{
	...
	if (gxl)
	{
		...
		GXLIGHT_PROPERTY	lightProperty = {0};
...
void CTool::OnClickLight(POINT* ptCursor,UINT nFlags)
{
	if (m_dwEditMode & EDIT_TYPE_CREATE)
	{
		if (!m_bMapBuild)
		{
			MessageBox(NULL,"���� ����� �Ŀ� ����Ʈ�� ���� �� �ֽ��ϴ�.","Error",MB_OK);
			return;
		}
		LIGHT_DESC	light;
...
void CTool::ReplaceTile(LPCTSTR szFileName,DWORD dwTileIndex)
{
	if (m_pHField)
		m_pHField->ReplaceTile(LPTSTR(szFileName),dwTileIndex);
}
...
void CTool::SaveMap(char* szFileName)
{
	...
	{
		MessageBox(NULL,"StaticModel�� HField �� �� �����ϴ�.","Error",MB_OK);
		return;
	}
	...
	if (!fp) ASSERT("CTool::SaveMap() - ���� �����Ͱ� �����ϴ�" );
...
*/
	...
	///--------------------------------------------------
	/// GXZones
	fprintf(fp,"%s \t %u",PID_GX_ZONE, MAX_ZONE_NUM);
	fprintf(fp,"\n");
	fprintf(fp,"%s","{ \n");

	for (DWORD i=0; i<MAX_ZONE_NUM; i++)
	{
		fprintf(fp,"\t %u \t %u\n", m_gxZoneData[i].dwMoveChar, m_gxZoneData[i].dwMoveMon);
	}
	fprintf(fp,"%s","} \n");
	...
	while (fscanf(fp,"%s",buf)!=EOF)
	{
		...
		if (!lstrcmp(buf,PID_GX_OBJECT))
		{
			...
			for (DWORD i=0; i<dwGXObjectsNum; i++)
			{
				...
				/// ���⼭ dwFlag�߿� Lock�� ���������� �Ʒ����� ��ġ������ �ȸԾ 0,0,0 �� ��ġ�ϰ� �ȴ�
				GXOBJECT_HANDLE gxo = m_pExecutive->CreateGXObject(buf,NULL,NULL,dwFlag);

				BOOL l_LockFlag = FALSE;

				if (gxo)
				{
					// Lock�÷��װ� �ִ� �༮�̶��
					if ( m_pExecutive->IsLockTransform(gxo) )
					{
						// �ϴ� Lock�÷��׸� ����
						m_pExecutive->UnLockTransform(gxo);
						l_LockFlag = TRUE;
		...
		if (!lstrcmp(buf,PID_GX_LIGHT))
		{
			...
			for (DWORD i=0; i<dwGXLightsNum; i++)
			{
				...
				m_pExecutive->CreateGXLight(&lightDesc,NULL,NULL,0xffffffff,NULL,dwFlag);
			}
			fscanf(fp,"%s",buf);		// }
		}
		...
		if (!lstrcmp(buf,PID_GX_ZONE))
		{
			DWORD	dwGXZoneNum;
			fscanf(fp,"%u",&dwGXZoneNum);

			fscanf(fp,"%s",buf);		// {
			for (DWORD i=0; i<dwGXZoneNum; i++)
			{
				DWORD		moveChar, moveMon;
				fscanf(fp,"%u",&moveChar);
				fscanf(fp,"%u",&moveMon);

				if (dwGXZoneNum < MAX_ZONE_NUM)
				{
					m_gxZoneData[i].dwMoveChar = moveChar;
					m_gxZoneData[i].dwMoveMon = moveMon;

					m_dwZoneNum = dwGXZoneNum;
				}
			}
			fscanf(fp,"%s",buf);		// }
		}
...
void CTool::LoadLights(FILE* fp)
{
	...
	for (DWORD i=0; i<dwGXLightsNum; i++)
	{
		...
		m_pExecutive->CreateGXLight(&lightDesc,NULL,NULL,0xffffffff,NULL,GXLIGHT_TYPE_STATIC);
	}
	fscanf(fp,"%s",buf);		// }
}
...
void CTool::OnSpotSplitterMove( UINT nFlags, POINT *pt )
{
	...
	if( nFlags & MK_LBUTTON )
	{
		if (m_pHField->GetTilePosWithScreenCoord(&m_v3Clicked,&fDist,&dwPosX,&dwPosY,pt,m_dwCurrentViewportIndex))
		{
			...
			WORD wSpotID = m_pSpotSplitter->pSectionList->GetCount();	//���� �Ҵ��� SpotID + 1�� �ǰ���..
...
void CTool::OnSectionSplitterUp( UINT nFlags, POINT *pt )
{
	...
	Section_Info*	pSec_Info ;			// Section Info ����.
...
void CTool::DeleteSpot( Section_Info* pSection_Info )
{
	...
	while( pos )
	{
		...
		cnt++;
		pSection_Info2->wSectionNum	= cnt;		//���ο� ID�� ���� 
...
//	m_pExecutive->GetRenderer()->SetLightMapFlag(0);
	...
	//--------------------------------------------------------------------------------------------------------
	//--								���⼭���� MoveSpot �ҷ����� 	(Added by KBS)						--
	//--------------------------------------------------------------------------------------------------------
	WORD	wSpotNum;
	Section_Info*	pSpotInfo;
	int i=0;

	fread( &wSpotNum, sizeof( WORD ), 1, fp );	//MoveSpot�� ����� ���� 
	...
	//--------------------------------------------------------------------------------------------------------
	//--								���⼭���� StartSpot �ҷ����� 	(Added by KBS)						--
	//--------------------------------------------------------------------------------------------------------
	fread( &wSpotNum, sizeof( WORD ), 1, fp );	//StartSpot�� ����� ���� 
...
void CTool::OnStartSpotSplitterMove( UINT nFlags, POINT *pt )
{
	...
	if( nFlags & MK_LBUTTON )
	{
		if (m_pHField->GetTilePosWithScreenCoord(&m_v3Clicked,&fDist,&dwPosX,&dwPosY,pt,m_dwCurrentViewportIndex))
		{
			...
			WORD wSpotID = m_pStartSplitter->pSectionList->GetCount();	//���� �Ҵ��� SpotID + 1�� �ǰ���..
...
BOOL CTool::ExportTileTableForCorum(char* szFileName)
{
	...
	ttd.dwTileManyX			=	hfDesc.dwTileNumX;								// x ���� �ְ�.
	ttd.dwTileManyZ			=	hfDesc.dwTileNumZ;								// y ���� �ְ�.
	ttd.dwFaceSize			=	(DWORD)hfDesc.fTileSize;
	ttd.dwFacesNumPerObjAxis=	hfDesc.dwFacesNumPerObjAxis;
	ttd.wSectionMany	=	m_pSectionSplitter->pSectionList->GetCount();	// �� Section ���� �ְ�.
	...
	if( ttd.wSectionMany != 0 )
	{
		...
		for( i=0; pos != NULL; i++ )
		{
			...
			ttd.pSectionLinkInfo[i].wSectionNum = i+1;	// Section Num�� 1���ͽ����Ѵ�.
			ttd.pSectionLinkInfo[i].x1			= static_cast<WORD>(pSection_Info->rcSection.left);
			ttd.pSectionLinkInfo[i].y1			= static_cast<WORD>(pSection_Info->rcSection.top);
			ttd.pSectionLinkInfo[i].x2			= static_cast<WORD>(pSection_Info->rcSection.right);
			ttd.pSectionLinkInfo[i].y2			= static_cast<WORD>(pSection_Info->rcSection.bottom);
		}
	...
	// ���� ����...
	fwrite(&ttd.dwTileManyX ,sizeof(DWORD),1,fp);	// x���� ����.
	fwrite(&ttd.dwTileManyZ ,sizeof(DWORD),1,fp);	// z���� ����.
	fwrite(&ttd.dwFaceSize ,sizeof(DWORD),1,fp);	
	fwrite(&ttd.dwFacesNumPerObjAxis ,sizeof(DWORD),1,fp);	
	fwrite(hfDesc.pwTileTable,sizeof(WORD),hfDesc.dwTileNumX*hfDesc.dwTileNumZ,fp);	// Ÿ�� ���� ����.
	
	fwrite(&ttd.wSectionMany, sizeof(WORD),1,fp);	// ���Ǽ� ����.
	...
	//--------------------------------------------------------------------------------------------------------
	//--								���⼭���� MoveSpot �����ϱ� 	(Added by KBS)						--
	//--------------------------------------------------------------------------------------------------------
	
	WORD	wSpotNum = m_pSpotSplitter->pSectionList->GetCount();
	fwrite( &wSpotNum, sizeof(WORD), 1, fp );	//MoveSpot ���� 
	...
	while( pos )
	{
		...
		fwrite( &pSection_Info2->rcSection, sizeof(RECT), 1, fp );		//Spot Rect ���� 
	}
	//------------------------------------------------------------------------------------------------------------

	//--------------------------------------------------------------------------------------------------------
	//--								���⼭���� StartSpot �����ϱ� 	(Added by KBS)						--
	//--------------------------------------------------------------------------------------------------------
	WORD	wStartNum = m_pStartSplitter->pSectionList->GetCount();
	fwrite( &wStartNum, sizeof(WORD), 1, fp );	//MoveSpot ���� 
...
void CTool::RenderTileText()
{
	if(m_pHField)
	{
		...
		if (m_pHField->GetTilePosWithScreenCoord(&vPos,&fDist,&dwPosX,&dwPosY,&m_ptMousePos, m_dwCurrentViewportIndex))
		{
			...
			rect.left -= 1;
			m_pFont->DrawText(szTileText,len,&rect,0xff000000,CHAR_CODE_TYPE_ASCII, 0, NULL);
			...
			m_pFont->DrawText(szTileText,len,&rect2,0xffffff00,CHAR_CODE_TYPE_ASCII, 0, NULL);
		}
	}
}

void CTool::SetCurrentMapFileName(LPCTSTR name)
{
	m_szCurrentMapFileName = name;
}

LPCTSTR CTool::GetCurrentMapFileName() const
{
	return m_szCurrentMapFileName;
}
...
void CTool::SetCurrentTilePalleteFileName(LPCTSTR name)
{
	m_szCurrentTilePaletteFileName = name;
}

LPCTSTR CTool::GetCurrentTilePalleteFileName() const
{
	return m_szCurrentTilePaletteFileName;
}

void CTool::SetCurrentTileSetFileName(LPCTSTR name)
{
	m_szCurrentTileSetFileName = name;
}

LPCTSTR CTool::GetCurrentTileSetFileName() const
{
	return m_szCurrentTileSetFileName;
}
...
/// ���� �ܾƿ���
void CTool::ZoomInOut( float p_Value )
{
	m_Mouse->MoveCamera_Forward( p_Value );
...
CHistoryManager& CTool::GetHistoryManager() const
{
	return *mHistoryManager;
}

void CTool::UpdateTile(LPCTSTR path, DWORD index)
{
	if(0 == m_pPalette)
	{
		return;
	...
	m_pPalette->UpdateTile(path, index);
}