...
//		PrintfDebugString("%f\n",args.matWorldForPhysique._11);
	...
	*pPrvArgs = args;
    ...
    for (i=0; i<m_dwTransformSetNum; i++)
	{
		...
		if (pParentMat)
		{
			// iros : 2009.08.24 계층구조에서 Scale 적용문제.

			if (pModelStatus->bAxisAlignOK)
				*pResult = *pTransformMat; // pTransformMat는 앞에서 계산을 다 해둔 Matrix
			else
				*pResult = *pParentMat; // 무조건 부모 Matrix에 종속 
...
BOOL __stdcall CoModel::GetCollisionMeshObjectDesc(COLLISION_MESH_OBJECT_DESC* pColMeshObjDesc,DWORD dwModelRefIndex,DWORD dwObjIndex)
{
	...
	if(OBJECT_TYPE_MESH != m_ppObjectList[dwObjIndex]->GetObjectType())
		goto lb_return;
...
DWORD __stdcall	CoModel::GetMeshObjectWithRay(VECTOR3* pv3IntersectPoint,float* pfDist,DWORD* pdwFaceGroupIndex,DWORD dwRefIndex,MATRIX4* pParent,MATRIX4* pTransform,DWORD dwFrame,DWORD dwMotionIndex,VECTOR3* pv3RayPos,VECTOR3* pv3RayDir,DWORD dwFlag)
{
	DWORD dwResultIndex = GetMeshObjectWithRayAndMatrix(
						pv3IntersectPoint,
						pfDist,
						pdwFaceGroupIndex,
						dwRefIndex,
...
DWORD __stdcall	CoModel::GetMeshObjectWithRayAndMatrix(VECTOR3* pv3IntersectPoint,float* pfDist,DWORD* pdwFaceGroupIndex,DWORD dwRefIndex,MATRIX4* pParent,MATRIX4* pTransform,DWORD dwFrame,DWORD dwMotionIndex,VECTOR3* pv3RayPos,VECTOR3* pv3RayDir,DWORD dwFlag)
{
	DWORD		i;
	...
	for (i=0; i<m_dwBoneObjectNum; i++)
	{
		if (IsCollisionMeshAndRay(&v3IntersectPoint,&fDist,pColMeshDesc,pv3RayPos,pv3RayDir))
		{
			if (fDist < fMinDist)
			{
				fMinDist = fDist;
...
lb_search_mesh:
	for (i=0; i<m_dwMeshObjectNum; i++)
	{
		...
		if (IsCollisionMeshAndRay(&v3IntersectPoint,&fDist,pColMeshDesc,pv3RayPos,pv3RayDir))
		{
			...
			{
				if (!m_pMeshObjectList[i].IsCollisionMeshAndRay(&v3IntersectPointPerFace,&fDistPerFace,&dwFaceGroupIndex,pv3RayPos,pv3RayDir,pMatrixEntry+m_pMeshObjectList[i].GetIndex()))
					continue;
...
DWORD __stdcall CoModel::GetMeshObjectWithBoundingBox(VECTOR3* pv3IntersectPoint,float* pfDist,DWORD* pdwFaceGroupIndex,DWORD dwRefIndex,MATRIX4* pParent,MATRIX4* pTransform,DWORD dwFrame,DWORD dwMotionIndex,COLLISION_MESH_OBJECT_DESC* pSrcColDesc,DWORD dwFlag)
{
	DWORD		i;
	...
	for (i=0; i<m_dwMeshObjectNum; i++)
	{
		...
		if (PICK_TYPE_PER_FACE == dwFlag )
		{
			if(!m_pMeshObjectList[i].IsCollisionMeshAndBoundingBox(&v3IntersectPointPerFace,&fDistPerFace,&dwFaceGroupIndex,pSrcColDesc,pMatrixEntry+m_pMeshObjectList[i].GetIndex()))
				continue;
