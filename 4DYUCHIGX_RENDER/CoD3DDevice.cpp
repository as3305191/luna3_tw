...
BOOL __stdcall CoD3DDevice::GetD3DDevice(REFIID refiid,void** ppVoid)
{
	...
	*ppVoid = NULL;
	...
	if (refiid != IID_IDirect3DDevice9)
		goto lb_return;
...
void* __stdcall CoD3DDevice::GetD3DDevice()
{
	return m_pD3DDevice;
}
...
//trustpak 2005/06/08			// �Ʒ� ��ɵ��� ���� ���ӿ��� ����ؼ��� �ȵ˴ϴ�. ���� ���ؼ��� �����ϴ� �����.
...
//	m_d3dpp.FullScreen_RefreshRateInHz = 85;
	...
	DWORD MSQuality = 0;
	D3DMULTISAMPLE_TYPE MSTypeHandled = D3DMULTISAMPLE_NONE;
		...
		while( fgets( buffer, sizeof( buffer ), file ) )
			{
				...
				else if( ! strcmpi( "aliashing", token ) )
				{
					token = strtok( NULL, separator );
					if( ! strcmpi("0", token))
					{
						MSTypeHandled = D3DMULTISAMPLE_NONE;
					}
					else if( ! strcmpi("2", token))
					{
						MSTypeHandled = D3DMULTISAMPLE_2_SAMPLES;
					}
					else if( ! strcmpi("4", token))
					{
						MSTypeHandled = D3DMULTISAMPLE_4_SAMPLES;
					}
					else if( ! strcmpi("8", token))
					{
						MSTypeHandled = D3DMULTISAMPLE_8_SAMPLES;
					}
					else if( ! strcmpi("16", token))
					{
						MSTypeHandled = D3DMULTISAMPLE_8_SAMPLES;
					}
					else
	...
	if( MSTypeHandled != D3DMULTISAMPLE_NONE )
	{
		if( SUCCEEDED( m_pD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_d3dpp.BackBufferFormat, TRUE, MSTypeHandled, &MSQuality) ) )
			SetMultiSample(MSTypeHandled);
		else
		{
			for( int i = 8; i > 1; i--)
			{
				if( SUCCEEDED( m_pD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_d3dpp.BackBufferFormat, TRUE, D3DMULTISAMPLE_TYPE(i), &MSQuality) ) )
				{
					SetMultiSample(D3DMULTISAMPLE_TYPE(i));
					break;
				}
			}
		}
		...
		/*char msaaText[128];
		sprintf( msaaText, "Multi Sample Type = x%d", GetMultiSample() );
		MessageBox( NULL, msaaText, "MSAA AMOUNT", MB_OK );*/

		m_d3dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;
		m_d3dpp.MultiSampleType = GetMultiSample();
		m_d3dpp.MultiSampleQuality = 0;//MSQ;
	...
	//trustpak 2005/07/08
	//m_TextBuffer.Initialize(MAX_RENDER_TEXTBUFFER_SIZE);
	m_TextBuffer.Initialize(MAX_RENDER_TEXTBUFFER_SIZE * 2, this);
	//
	...
	// ¿øÀ» ±×¸®±â À§ÇÑ ½áÅ¬ Å¬·¡½º ÃÊ±âÈ­.
	m_Circle.Initialize(DEFULAT_CIRCLE_PIECES_NUM);


	// µðÆúÆ® ´ÙÀÌ³ª¹Í ¶óÀÌÆ®¸¦ ÇÏ³ª ¸¸µé¾îµÐ´Ù..
	m_dwDefaultLightIndex = CreateDynamicLight(16,0x000000ff,NULL);
...
LPDIRECT3DTEXTURE9 CoD3DDevice::GetDynamicLightTexture(DWORD dwIndex)
{
...
//	RENDER_MESHOBJECT_DESC* pItem = (RENDER_MESHOBJECT_DESC*)&pMeshObj;
	...
	if (m_RenderQ.InsertItem((char*)&itemDesc,fDistance))
		return TRUE;
	else
	{
#ifdef _DEBUG
		_ASSERT(false);
		OutputDebugString("CoD3DDevice::RenderMeshObject() : m_RenderQ is FULL~!! ");
#endif
		return FALSE;
	}
}
...
BOOL __stdcall CoD3DDevice::RenderFont(IDIFontObject* pFont,TCHAR* str,DWORD dwLen,RECT* pRect,DWORD dwColor,CHAR_CODE_TYPE type,int iZOrder,DWORD dwFlag)
{
	...
	if (pRect->right - pRect->left > (long)m_pFontCache->GetMaxWidth())
	{
		memset(txt,0,sizeof(txt));
	...
	if (pRect->bottom - pRect->top > (long)m_pFontCache->GetMaxWidth())
	{
		memset(txt,0,sizeof(txt));
...
//		pRect = &m_rcDirty;
	...
	m_pD3DDevice->GetRenderTarget(D3DCREATE_PUREDEVICE, &pDest);
...
{
//	LPDIRECT3DSURFACE9			pDefaultRenderSurface;		
//	LPDIRECT3DSURFACE9			pDefaultZBuffer;
	...
	// D3DPOOL_DEFAULT�� ����ϴ� ���ؽ�����,�ε������� ĳ���̴�.
	m_VBCache.Cleanup();
	m_IBCache.Cleanup();

	m_TextBuffer.OnLostDevice();
	...
	HRESULT rt = m_pD3DDevice->TestCooperativeLevel();    // ���� ����
	if (bTest)
	{
		while( rt != D3DERR_DEVICENOTRESET )
		{
			Sleep( 30 );									// 30 ����.Ofon
			rt = m_pD3DDevice->TestCooperativeLevel();    // ���¸� �ٽ� ����
		}

		if (rt != D3DERR_DEVICENOTRESET)
			return;
	}

	HRESULT hr;

	if ((hr=m_pD3DDevice->Reset(&m_d3dpp)) != D3D_OK)
	{
#ifdef _DEBUG
		OutputDebugString("CoD3DDevice::ResetDevice(BOOL bTest) : Device Reset Failed! \n");
#endif
	}
...
//	m_pFont->BeginRender();
	if (m_rcSysInfo1.right - m_rcSysInfo1.left > 0)
		m_pFont->DrawText(m_szSysInfo1,lstrlen(m_szSysInfo1),&m_rcSysInfo1,0xff00ff00,CHAR_CODE_TYPE_ASCII,0,NULL);

	if (m_rcSysInfo2.right - m_rcSysInfo2.left > 0)
		m_pFont->DrawText(m_szSysInfo2,lstrlen(m_szSysInfo2),&m_rcSysInfo2,0xff00ff00,CHAR_CODE_TYPE_ASCII,0,NULL);
...
void __stdcall CoD3DDevice::RenderPoint(VECTOR3* pv3Point,DWORD dwColor)
{
	...
	*(VECTOR3*)&v3Point.x = *pv3Point;
	...
	m_pD3DDevice->SetVertexShader(NULL);
	m_pD3DDevice->SetFVF(D3DFVF_D3DLVERTEX);
...
void __stdcall CoD3DDevice::RenderCircle(VECTOR2* pv2Point,float fRs,DWORD dwColor)
{
	...
	m_pD3DDevice->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_DISABLE );
	m_pD3DDevice->SetVertexShader(NULL);
	m_pD3DDevice->SetFVF(D3DFVF_4DVERTEX);
...
{
	LPDIRECT3DSURFACE9 pPrimarySurface;
	LPDIRECT3DSURFACE9 pTempSurface;
	D3DSURFACE_DESC  desc;

	if (D3D_OK != m_pD3DDevice->GetRenderTarget(0, &pPrimarySurface))
	{
		return FALSE;
	}
	...
	if (D3D_OK != m_pD3DDevice->CreateTexture(desc.Width, desc.Height, 1, 0, desc.Format, D3DPOOL_SYSTEMMEM, &pTempTex, NULL ))
	{
		return FALSE;
	}
	...
	// CopyRect(DX8) ��ſ� UpdateSurface()�� ������, �� ���� ����Ÿ�ٿ��� �����ؾ��ϱ� ������ ����ϴ� �Լ�
	if (D3D_OK != m_pD3DDevice->GetRenderTargetData(pPrimarySurface, pTempSurface))
	{
		return FALSE;
	}

	D3DLOCKED_RECT lockRect;
	pTempSurface->LockRect(&lockRect,NULL,D3DLOCK_DISCARD | D3DLOCK_READONLY );
...
/*	__asm 
	...
	pMatResult->_31	=	fXTrans;		// count2�� 3*3����� ����
