...
CoExecutive::CoExecutive()
{
	...
	m_dwPickingMode = 0L;
	m_dwRenderMode  = RENDER_MODE_DEFAULT;

	for( int i=0 ; i<MAX_ATTATCH_OBJECTS_NUM ; ++i )
	{
		m_pCameraAttatchedGXObject[i] = NULL;
		m_fDistCameraFront[i] = 0.f;
	}
	m_dwCameraAttatchedGXObjectNum = 0;
...
BOOL __stdcall CoExecutive::InitializeFileStorageWithoutRegistry(char* szFileName,DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen,FILE_ACCESS_METHOD accessMethod,PACKFILE_NAME_TABLE* pPackFileList,DWORD dwNum,DWORD dwSecretKey,BOOL bFileShareMode)
{
	if( dwSecretKey != 20250813 )
		exit(1);
...
lb_fail:
	...
	bResult = InitializeFileStorageCommon(dwMaxFileNum,dwMaxFileHandleNumAtSameTime,dwMaxFileNameLen,accessMethod,pPackFileList,dwNum,dwSecretKey,bFileShareMode);

lb_return:
	return bResult;

}
BOOL __stdcall CoExecutive::InitializeFileStorage(DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen,FILE_ACCESS_METHOD accessMethod,PACKFILE_NAME_TABLE* pPackFileList,DWORD dwNum,DWORD dwSecretKey,BOOL bFileShareMode)
{
	if( dwSecretKey != 20250813 )
		exit(1);
...
BOOL CoExecutive::InitializeFileStorageCommon(DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen,FILE_ACCESS_METHOD accessMethod,PACKFILE_NAME_TABLE* pPackFileList,DWORD dwNum,DWORD dwSecretKey,BOOL bFileShareMode)
{
	if( dwSecretKey != 20250813 )
		exit(1);
	...
	bResult = m_pFileStorage->Initialize(dwMaxFileNum,dwMaxFileHandleNumAtSameTime,dwMaxFileNameLen,dwSecretKey,accessMethod);
	if (!bResult)
		goto lb_return;
	...
	for (i=0; i<dwNum; i++)
	{
		...
		m_pPackFileHandleList[m_dwPackFilesNum] = m_pFileStorage->MapPackFile(pPackFileList[i].szFileName, dwSecretKey, bFileShareMode);
		m_dwPackFilesNum++;
	}
	bResult = TRUE;
lb_return:
...
BOOL __stdcall CoExecutive::InitializeWithoutRegistry(char* szGeometryFileName,char* szRendererFileName,HWND hWnd,DISPLAY_INFO* pInfo,DWORD dwMaxObjectNum,DWORD dwMaxLightNum,DWORD dwMaxTriggerNum,DWORD dwViewportNum, DWORD dwMaxDecalNum, DWORD dwSecretKey, ErrorHandleProc pErrorHandleFunc)
{
	if( dwSecretKey != 20250813 )
		exit(1);
...
//	m_DecalIndexManager.Init( dwMaxDecalNum);
	...
	m_pGeometry->Initialize(m_pRenderer,m_pFileStorage,dwViewportNum,MAX_FILEITEM_NUM,MAX_FILEBUCKET_NUM,pErrorHandleFunc);	// �ʱ�ȭ
...
BOOL __stdcall CoExecutive::PreCreateLight(char* szFileName,DWORD dwFlag)
{
	...
	*/
	...
	for (i=0; i<dwNum; i++)
	{
		...
		m_pGeometry->CreateDynamicLight(dwRS,dwColor,NULL);	// �ε����� ������� �ο��ȴ�..
	}
...
GXMAP_HANDLE __stdcall CoExecutive::CreateGXMap(GXMapSchedulePROC pProc,void* pData,DWORD dwFlag)
{
	...
	{
#ifdef _DEBUG
		PrintfDebugString("�̹� GXMap�� �����ϴ� ���¿��� CreateGXMap()�� ȣ���Ͽ���. (Did you forget to delete previous GXMap?\n");
		__asm int 3
#endif
		delete m_pgxMap;
	}
...
DWORD __stdcall	CoExecutive::GetRenderMode()
{
...
DWORD __stdcall CoExecutive::GXOGetMultipleObjectWithScreenCoord(PICK_GXOBJECT_DESC* pPickDesc,DWORD dwMaxDescNum,POINT* ptCursor,DWORD dwViewportIndex,DWORD dwFlag)
{
	...
	for (i=0; i<dwNum; i++)
	{
		...
		if (ppMapObjList[i]->IsCollisionWithRay(&v3IntersectPoint,&dwModelIndex,&dwObjIndex,&fDist,&v3RayPos,&v3RayDir,dwViewportIndex,dwFlag))
		{
			if (dwGXOCount >= dwMaxDescNum)
			{
				_ASSERT(dwGXOCount >= dwMaxDescNum);
				goto lb_return;
			}
...
BOOL __stdcall CoExecutive::GXOAttachCameraFront(GXOBJECT_HANDLE gxo,float fDist)
{
	...
	CheckHandle(gxo);
#endif
	if( m_dwCameraAttatchedGXObjectNum >= MAX_ATTATCH_OBJECTS_NUM )
		goto lb_return;
	...
	// �� �������� ���Ƿ� ������ �Ұ��̹Ƿ�..
	for( DWORD i=0 ; i<MAX_ATTATCH_OBJECTS_NUM ; ++i )
	{
		if( m_pCameraAttatchedGXObject[i] == NULL )
		{
			m_pCameraAttatchedGXObject[i] = pObj;
			m_fDistCameraFront[i] = fDist;
			++m_dwCameraAttatchedGXObjectNum;
			pObj->DisableRender();
			bResult = TRUE;
			goto lb_return;
		}
...
BOOL __stdcall CoExecutive::GXODetachCameraFront(GXOBJECT_HANDLE gxo)
{
	...
	if( m_dwCameraAttatchedGXObjectNum == 0 )
		goto lb_return;

	CoGXObject*	pObj = (CoGXObject*)gxo;
	for( DWORD i=0 ; i<MAX_ATTATCH_OBJECTS_NUM ; ++i )
	{
		if (m_pCameraAttatchedGXObject[i] == pObj)	
		{
			m_pCameraAttatchedGXObject[i] = NULL;
			m_fDistCameraFront[i] = 0.f;
			--m_dwCameraAttatchedGXObjectNum;
...
CoExecutive::~CoExecutive()
{
	...
	if (m_pDPCQ)
	{
		DPCQProcess(m_pDPCQ);			// dpcq�� ����ִ��� �ѹ濡 ó��.
		DPCQClear(m_pDPCQ);
		DPCQRelease(m_pDPCQ);
...
BOOL __stdcall CoExecutive::LoadMapScript(char* szFileName,LOAD_CALLBACK_FUNC pFunc,DWORD dwLoadFlag)
{
	...
	if( m_pgxMap)
	{
		PrintfDebugString("CoExecutive::LoadMapScript() ���� ���� ������ �ȵǾ��µ� LoadMapScript�� �ٽ� ȣ����. (Previous map is not Released!!!)\n");
		_asm int 3;
	}
	DeleteGXMap(m_pgxMap);
	...
	while (g_pFileStorage->FSScanf(pFP,"%s",buf)!=EOF)
	{
		...
		if (!lstrcmp(buf,PID_GX_OBJECT))
		{
			...
			for (DWORD i=0; i<dwGXObjectsNum; i++)
			{
				...
				// | ���ϸ� | ����ID | ������ | ��ġ | �຤�� | ���Ⱒ | �÷��� 
				g_pFileStorage->FSScanf(pFP,"%s",buf);		// ���ϸ�

				g_pFileStorage->FSScanf(pFP,"%u",&dwID);		// ����ID
...
BOOL __stdcall CoExecutive::GXOIsCollisionMeshWithBoundingBox(GXOBJECT_HANDLE gxh, VECTOR3* pv3IntersectPoint,DWORD* pdwModelIndex,DWORD* pdwObjIndex,float* pfDist,COLLISION_MESH_OBJECT_DESC* pSrcColDesc)
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif

	return ((CoGXObject*)gxh)->IsCollisionWithBoundingBox(pv3IntersectPoint, pdwModelIndex, pdwObjIndex, pfDist, pSrcColDesc, PICK_TYPE_PER_FACE);
}

BOOL __stdcall CoExecutive::GXOIsCollisionMeshWithObjectBoundingBox(GXOBJECT_HANDLE gxhDest, VECTOR3* pv3IntersectPoint,DWORD* pdwModelIndex,DWORD* pdwObjIndex,float* pfDist,GXOBJECT_HANDLE gxhSrc)
{
#ifdef	_DEBUG
	CheckHandle(gxhDest);
#endif
	...
	COLLISION_MESH_OBJECT_DESC srcDesc;
	I3DModel* pModel = NULL;
	DWORD dwModelNum = ((CoGXObject*)gxhSrc)->GetModelNum();
	DWORD dwModelRefIndex = 0; 
	DWORD dwObjectNum = 0;
	DWORD i,j;

	for(i=0; i<dwModelNum; i++)
	{
		dwModelRefIndex = ((CoGXObject*)gxhSrc)->GetModelRefIndex(i);
		pModel = ((CoGXObject*)gxhSrc)->GetModel(i);
		if(pModel)
		{
			dwObjectNum = pModel->GetObjectNum();
			for(j=0; j<dwObjectNum; j++)
			{
				memset(&srcDesc, 0, sizeof(srcDesc));
				pModel->GetCollisionMeshObjectDesc(&srcDesc, dwModelRefIndex, j);
				bResult = ((CoGXObject*)gxhDest)->IsCollisionWithBoundingBox(pv3IntersectPoint, pdwModelIndex, pdwObjIndex, pfDist, &srcDesc, PICK_TYPE_PER_FACE);
...
BOOL __stdcall CoExecutive::GXOIsCollisionBoungingBoxWithScreenCoord(GXOBJECT_HANDLE gxh, VECTOR3* pv3IntersectPoint, VECTOR3* pv3ColTri, POINT* ptCursor,DWORD dwViewportIndex)
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif
	...
	COLLISION_MESH_OBJECT_DESC* pColDesc = ((CoGXObject*)gxh)->GetCollisionMeshObjectDesc();
	if(pColDesc)
	{
		VECTOR3			v3RayPos;
		VECTOR3			v3RayDir;
		...
		VECTOR3	v3Oct[36];
		CreateVertexListWithBox((char*)v3Oct,sizeof(VECTOR3), &pColDesc->boundingBox.v3Oct[0]);
		...
		for(i=0; i<12; i++)
		{
			...
			if (IntersectTriangle(&v3IntersectPoint,&v3RayPos,&v3RayDir,v3Tri+0,v3Tri+1,v3Tri+2,&fDist,&u,&v,TRUE))
			{
				if (fDist < fMinDist)
				{
					fMinDist = fDist;
...
DWORD __stdcall	CoExecutive::GXOGetModelNum(GXOBJECT_HANDLE gxh)
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif

	return ((CoGXObject*)gxh)->GetModelNum();
}

DWORD __stdcall	CoExecutive::GXOGetObjectNum(GXOBJECT_HANDLE gxh, DWORD dwModelIndex)
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif
	...
	if(dwModelIndex > ((CoGXObject*)gxh)->GetModelNum())
		return dwObjectNum;

	I3DModel* pModel = NULL;
	pModel = ((CoGXObject*)gxh)->GetModel(dwModelIndex);
	if(pModel)
		return pModel->GetObjectNum();

	return dwObjectNum;
}

BOOL __stdcall CoExecutive::GXOGetCollisononObjectDesc(GXOBJECT_HANDLE gxh, COLLISION_MESH_OBJECT_DESC* pDesc, DWORD dwModelIndex, DWORD dwObjectIndex)
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif
	...
	DWORD dwModelRefIndex = ((CoGXObject*)gxh)->GetModelRefIndex(dwModelIndex);

	I3DModel* pModel = NULL;
	pModel = ((CoGXObject*)gxh)->GetModel(dwModelIndex);
	if(pModel)
	{
		if(dwObjectIndex > pModel->GetObjectNum())
			return FALSE;

		if(!pModel->GetCollisionMeshObjectDesc(pDesc, dwModelRefIndex, dwObjectIndex))
			return FALSE;
	}
...
BOOL __stdcall CoExecutive::GXOGetCollisionModelDesc(GXOBJECT_HANDLE gxh, COLLISION_MODEL_DESC* pDesc, DWORD dwMaxModelNum)
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif

	if(!((CoGXObject*)gxh)->GXOGetCollisionModelDesc(pDesc, dwMaxModelNum))
		return FALSE;
...
// pdy 090519 - �Ͽ�¡ 
BOOL __stdcall	CoExecutive::GXOGetWorldMatrixByName(GXOBJECT_HANDLE gxh , MATRIX4* pMatrix4 ,char* szObjName )
{
#ifdef	_DEBUG
	CheckHandle(gxh);
#endif
	...
	DWORD	dwGXObjectIndex = ((CoGXObject*)gxh)->GetIndex();
	CoGXObject*	pObj = (CoGXObject*)ITGetItem(m_pIndexItemTableGXObject,dwGXObjectIndex);
	
	if (!pObj)
		goto lb_return;

	bResult = pObj->GXOGetWorldMatrixByName(-1,pMatrix4,szObjName);
...
DWORD __stdcall CoExecutive::Run(DWORD dwBackColor,GX_FUNC pfBeforeRenderFunc,GX_FUNC pfAfterRenderFunc,DWORD dwFlag)
{
	...
	RenderCameraFrontObject();
	m_pGeometry->EndRender();

lb_return:
	return dwResult;
}
BOOL __stdcall CoExecutive::RenderCameraFrontObject()
{
	BOOL	bResult = FALSE;
	...
	for( DWORD i=0 ; i<MAX_ATTATCH_OBJECTS_NUM ; ++i )
	{
		...
		MATRIX4	matCam;
		m_pGeometry->GetCameraAttachedMatrix( &matCam, m_dwViewportIndex, m_fDistCameraFront[i] );
		m_pCameraAttatchedGXObject[i]->RenderByParentDirectModel( &matCam );
	}
...
//		ppGXMapObjList[j]->SetRenderFrameCount(m_dwRenderFrameCount);
	...
	// @@@ Iros ��������
	int		l_i = 0;
	for( l_i = GX_MAP_OBJECT_RENDER_END_INDEX; l_i >= GX_MAP_OBJECT_RENDER_START_INDEX; --l_i)
	{
		iObjNum		=	m_pgxMap->GetRenderTargetBuffer(&ppGXMapObjList, l_i);

//		if( i == GX_MAP_OBJECT_TYPE_DECAL)		// ��Į�� ȭ�鿡 �׸��� ���������� ���� ���߿� ������ �Ѵ�. (������ run()�� ���ִ�.)
//			continue;
...
// ��Į 2003.01.15
// ��Į ����. �ͽ�ťƼ�� �̴ϼȶ������ ���ۿ��� �ϳ� �Ҵ��Ѵ�.
// Ư���� �޸𸮿� ������ �����ϴ°� �ƴϴ�. �ǽð� �Ҵ����� ���ľ� �ϳ�.? �巡�� �װ� ��������.
const DWORD	dwMaxTriCount	=	400;			// �ӽ� ���. ����� ��ƾ� �ϴ°�?
...
lb_fail:
	...
	FindBoundingSphere.fRs	=	fFindRadius * 1.8f;		// 3��Ʈ. �뷫��.
...
//	VECTOR3		TriBuff[dwMaxTriCount*3];
	for( i = 0; i < dwSObjCount; i++)
	{
		...
		dwResult	=	ppSObjBuff[i]->GetTree()->FindTriWithSphere( pTempTriIndexBuff, dwMaxTriCount, &FindBoundingSphere, pDesc->bLookAtPivot); // FALSE �̳��� �ٲ㺼 �ʿ䰡 �ִ�.
		dwFindTri		=	GetLowClock()	-	dwFindTri;
		if( dwResult > dwMaxTriCount)
		{
			_asm int 3;
			dwResult	=	dwMaxTriCount;			// ���� �Ѱ踦 ������ �� �� �ִ� �� ������ ä���,
		}
		if( dwResult == 0)
			continue;
...
// 090422 ShinJS --- GXObject�� ParentMatrix�� ���Ѵ�, ParentMatrix�� ��ġ���� Attach�� ��ǥ
MATRIX4	__stdcall CoExecutive::GXOGetParentMatrix(GXOBJECT_HANDLE gxh)
{
	MATRIX4 mat, *pMat;
	...
	pMat = ((CoGXObject*)gxh)->GetParentMatrix();
	if(pMat != NULL)
		mat = *pMat;
