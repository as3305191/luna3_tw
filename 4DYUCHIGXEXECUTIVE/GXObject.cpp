...
BOOL __stdcall	CoGXObject::IsCollisionWithRay(VECTOR3* pv3IntersectPoint,DWORD* pdwModelIndex,DWORD* pdwObjIndex,float* pfDist,VECTOR3* pv3RayPos, VECTOR3* pv3RayDir,DWORD dwViewportIndex,DWORD dwFlag)
{
	...
	*pdwModelIndex = 0xffffffff;
	for (DWORD i=0; i<m_dwModelNum; i++)
	{
		...
		dwIndex = m_ModelList[i].pModel->GetMeshObjectWithRay(
			pv3IntersectPoint,
			pfDist,
			&dwFaceIndex,
...
BOOL __stdcall CoGXObject::IsCollisionWithBoundingBox(VECTOR3* pv3IntersectPoint,DWORD* pdwModelIndex,DWORD* pdwObjIndex,float* pfDist,COLLISION_MESH_OBJECT_DESC* pColDesc,DWORD dwFlag)
{
	DWORD	dwIndex = 0xffffffff;
	DWORD	dwFaceIndex;
	BOOL	bResult = FALSE;
	...
	*pdwModelIndex = 0xffffffff;
	for (DWORD i=0; i<m_dwModelNum; i++)
	{
		...
		dwIndex = m_ModelList[i].pModel->GetMeshObjectWithBoundingBox(
			pv3IntersectPoint,
			pfDist,
			&dwFaceIndex,
			m_ModelList[i].dwRefIndex,
...
BOOL __stdcall CoGXObject::IsCollisionWithObjectBoundingBox(VECTOR3* pv3IntersectPoint,DWORD* pdwModelIndex,DWORD* pdwObjIndex,float* pfDist, DWORD dwFlag)
{
	DWORD	dwIndex = 0xffffffff;
	DWORD	dwFaceIndex;
	BOOL	bResult = FALSE;
	COLLISION_MESH_OBJECT_DESC ColDesc;
	...
	*pdwModelIndex = 0xffffffff;
	for (DWORD i=0; i<m_dwModelNum; i++)
	{
		for(DWORD j=0; j<m_ModelList[i].pModel->GetObjectNum(); j++)
		{
			m_ModelList[i].pModel->GetCollisionMeshObjectDesc(&ColDesc, m_ModelList[i].dwRefIndex, i);
...
//	m_pParentGXObject->UpdateCollisionMesh();
	...
	*/
	...
	// 090422 ShinJS --- 부모를 가진 GXObject의 경우에도 자식이 있는경우 Render한다
	if (bResult)
	{
		for (DWORD j=0; j<m_dwChildGXObjectNum; j++)
		{
			m_pChildGXObject[j]->RenderByParent(m_pChildGXObject[j]->GetParentMatrix());
		}
	}
...
// pdy 090519 - 하우징 
BOOL		__stdcall	CoGXObject::GXOGetWorldMatrixByName(DWORD dwModelIndex, MATRIX4* pMatrix4 ,char* szObjName )
{
	BOOL		bResult = FALSE;
	DWORD		dwIndex = 0xffffffff;	
	I3DModel*	pModel = NULL;
...
lb_GetMatrix:
	bResult = pModel->GetWorldMatrix(pMatrix4,dwRefIndex,dwIndex);

lb_return:
	return bResult;
}

BOOL		__stdcall	CoGXObject::GXOGetCollisionModelDesc(COLLISION_MODEL_DESC* pDesc, DWORD dwMaxModelNum)
{
	I3DModel*	pModel = NULL;
	...
	for(i=0; i<m_dwModelNum; i++)
	{
		...
		if(!m_ModelList[i].pModel->GetCollisionModelDesc(&pDesc[i], m_ModelList[i].dwRefIndex))
		{
			return FALSE;
		}
	}
