...
void CGuildWar::RegistGuildWar( DWORD dwEnemyGuildIdx, DWORDEX dwMoney )
{
	...
	// �� ��� ��� ����
	{
		...
		for( const sGFWENEMY* enemy; (enemy = m_EnemyGuildTable.GetData())!=NULL; )
		{
			...
			if( guild )
			{
				...
				// MarkName�� ������ �뵵�� ����ϰ� �ִ�.
				dest.MarkName	= (DWORD)enemy->dwGFWMoney;
...
DWORDEX CGuildWar::GetWarMoney( DWORD dwEnemyGuildIdx )
{
...
BOOL CGuildWarManager::RegistGuildWar( DWORD dwGuildIdx1, DWORD dwGuildIdx2, DWORDEX dwMoney )
{
...
DWORDEX CGuildWarManager::GetWarMoney( DWORD dwGuildIdx, DWORD dwEnemyGuildIdx )
{
...
{
	const MSG_DWORD3_GUILD*	pmsg		= ( MSG_DWORD3_GUILD* )pMsg;
	...
	struct
	{
		void operator() ( CGuild* sourceGuild, CGuild* destGuild, DWORDEX money )
		{
			if( ! sourceGuild ||
				! destGuild )
			{
...
void CGuildWarManager::SuggestEndAccept( void* pMsg )
{
	...
	// �������� �ִٸ� ��ȯ������
	{
		const DWORDEX money = GetWarMoney( friendlyGuildIndex, enemyGuildIndex );
...
void CGuildWarManager::SendMoneyMsg( CPlayer* pPlayer1, CPlayer* pPlayer2, DWORDEX dwMoney, BYTE Protocol )
{
	if( !dwMoney )	return;
...
void CGuildWarManager::SendToAgentAddMoneyMsg( DWORD dwPlayerIdx, DWORDEX dwMoney )
{
	MSG_DWORDEX2 Msg;
	Msg.Category = MP_GUILD_WAR;
	Msg.Protocol = MP_GUILD_WAR_ADDMONEY_TOMAP;
	Msg.dweData1 = dwPlayerIdx;
	Msg.dweData2 = dwMoney;
...
void CGuildWarManager::AddEnemyToFriendly( Alliance& friendlyMap, Alliance& enemyMap )
{
	...
	MSG_DWORD3_GUILD messageToAgent;
