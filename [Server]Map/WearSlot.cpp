...
//{
//	// 횄횜?청횊짯 횉횎쩔채!
//	// Charactertable쩔징 ?횜쨈횂 횝짚쨘쨝 횝철쩔챵
...
ERROR_ITEM CWearSlot::InsertItemAbs(CPlayer * pPlayer, POSTYPE absPos, ITEMBASE* pItem, WORD state)
{
...

	{
		...
		message.mItemTransmog	= ITEMMGR->GetOption(*pItem).mTransmogIdx;
...
{
	// 아이템 분할, 병합, 강화 등 정보가 변경띘었을 때 호출띨. 인벤토리엝 든 아이템은 호출띘지 않음. 
	// 인벤토리엝 든 아이템엝서 이 메소드가 호출띠 경우는 정생젝으로 플레이어 생태가 갱신띘지 않을 것이다... 주의바람
