...
CSTRPath::CSTRPath()
{
	m_pSTRNodePool = new CPool<CSTRNode>;
	m_pSTRNodePool->Init(1000, 500,"CSTRNode");
	m_pStackNodePool = new CPool<CStackNode>;
...
void CSTRPath::TraversalChild(CSTRNode * parent, CSTRNode * child)
{
	...
	if ((check = GetInList(m_pOpenList, num)) != NULL ) 
	{
	...
	else if ( ( check = GetInList(m_pClosedList, num) ) != NULL ) 
	{
		parent->child[parent->childNum++] = check;
