...
void MP_CHARMsgParser(DWORD dwConnectionIndex, char* pMsg, DWORD dwLength)
{
	...
	case MP_CHAR_EMERGENCY_SYN:						Char_Emergency_Syn(pMsg) ;									break;
		// 080616 LUJ, �÷��̾��� HP ������ ��ȯ�Ѵ�
		//				������ Ŭ���̾�Ʈ�� ���� ������Ʈ�� ��, Ŭ���̾�Ʈ�� EffectUnit �ܰ迡�� HP �� ������ ���� ó���Ѵ�
		//				�̿� ���� ������ Ŭ���̾�Ʈ ���� �񵿱� ������ �߻��Ѵ�. �̸� ���� ���� Ŭ���̾�Ʈ����
		//				HP ���� ������Ʈ�� ��û�ϵ��� �Ѵ�
		// 090507 LUJ, ��û/��� ������Ʈ ����� HP�� �����ϵ��� ����
	...
	case MP_USERCONN_CHANGE_CHANNEL_IN_GAME_SYN:
		{
			...
			// 081218 LUJ, ������Ʈ���� �� �̵��� ���� ����� ��� ������Ʈ���� ���� ������ ���Եȴ�.
			//			�̸� ���� ���� ���ν��� ó�� �Ϸ� �� �����Ѵ�
			UpdatePlayerOnMapOutWithOutMoney( dwPlayerID, dwUserID, mapType, wChannel );
...
{
	const MSG_OPT* const receivedMessage = (MSG_OPT*)pMsg;
	const DWORD playerIndex = receivedMessage->dwObjectID;
	...
	const MONEYTYPE chargedMoney = receivedMessage->dweData3;
	const DWORD hideFlag = receivedMessage->dwData6;
	const BOOL moveType = LOWORD(receivedMessage->dwData7);
	...
	if(player->GetCharacterTotalInfo().HideFlag != hideFlag || player->GetCharacterTotalInfo().bWingsMove != moveType)
	{
		player->GetCharacterTotalInfo().HideFlag = hideFlag;
		player->GetCharacterTotalInfo().bWingsMove = moveType;
		...
		sendMessage.dwData1 = hideFlag;
		sendMessage.dwData2 = DWORD(moveType);
...
//			g_pServerSystem->RemovePlayer(pmsg->dwObjectID);
	...
	// 090227 ShinJS --- �̵�NPC ������ ������
	{
		...
		if( pMoveNpcInfo )
		{
			...
			{
				msg.dwData1		= MAKEDWORD( pInfo->MapNum, pInfo->wDir );						// �ʹ�ȣ, NPC ����
				msg.dwData2		= MAKEDWORD( pInfo->wNpcUniqueIdx, pInfo->wNpcJob );			// NPC Index, NPC Kind
				msg.dwData3		= MAKEDWORD( pInfo->vPos.x, pInfo->vPos.z );					// NPC ��ġ
				msg.dwData4		= DWORD( NPCMOVEMGR->IsReadyToMove( pInfo->wNpcUniqueIdx ) );	// �̵��غ���� ����
...
void UserConn_GameInPos_Syn(DWORD dwConnectionIndex, char* pMsg) 
{
	...
	if( pObject != NULL )	//�̹� �ִ�.
	{
		if( pObject->GetObjectKind() == eObjectKind_Player )	//�����ΰ�?
		{
//					g_pServerSystem->RemovePlayer(pmsg->dwObjectID);
...
void UserConn_Disconnected(DWORD dwConnectionIndex, char* pMsg) 
{
	...
	if( !pPlayer->IsNormalExit() )
	{
		...
		if( pPlayer->IsPKMode() )
		{
			...
			//---KES PK 071202
			PKMGR->PKModeDiePanelty( pPlayer ); //���� ������ ���� PK��� ���� �г�Ƽ�� �����Ѵ�.
		}
...
void MP_CHEATMsgParser(DWORD dwConnectionIndex, char* pMsg, DWORD dwLength)
{
	...
	// 081022 LYW --- Pet : �� ����/��Ÿ �׽�Ʈ�� ���Ͽ� Pet���� GOD ��带 �����Ѵ�.
...
void Cheat_Money_Syn(char* pMsg) 
{
	...
	if( pPlayer->GetMoney() > pmsg->dweData )
	{
		pPlayer->SetMoney(pPlayer->GetMoney() - pmsg->dweData, MONEY_SUBTRACTION, 0, eItemTable_Inventory, eMoneyLog_GetCheatMoney, 0);
	}
	else if( pPlayer->GetMoney() < pmsg->dweData )
	{
		pPlayer->SetMoney(pmsg->dweData - pPlayer->GetMoney(), MONEY_ADDITION, 0, eItemTable_Inventory, eMoneyLog_GetCheatMoney, 0);
	}
	
	LogCheat(pPlayer->GetID(),eCHT_Money,pmsg->dweData,0,0);		
}
...
// 080205 LYW --- MapNetworkMsgParser : �̹�Ʈ ����, �� �ʿ� ó�� �ǵ��� ����.
void Cheat_Event_Syn(char* pMsg) 
{
	if( !pMsg ) return ;														// �޽��� üũ.

	MSG_WORD2* pmsg = NULL ;
	pmsg = (MSG_WORD2*)pMsg ;													// �޽����� ��ȯ �޴´�.

	if( !pmsg ) return ;														// ��ȯ �� �޽��� üũ.

	WORD wEventType = 0 ;
	wEventType = pmsg->wData1 ;													// �̺�Ʈ ������ �޴´�.

	WORD wEventRate = 0 ;
	wEventRate = pmsg->wData2 ;													// �̺�Ʈ���� �޴´�.

	if( wEventRate > 3000 ) return ;											// �ִ� 30�� ����.

	// 080227 LYW --- MapNetworkMsgParser : EventRateFile�� ����ȴ� ���� �����Ѵ�.
	if( wEventType == eEvent_ExpRate )											// ����ġ�� �����Ѵ�.
	{
		gEventRate[eEvent_ExpRate] = wEventRate*0.01f;							
		gEventRateFile[eEvent_ExpRate] = wEventRate*0.01f;
	}
	else if( wEventType == eEvent_ItemRate )									// ������ ������� �����Ѵ�.
	{
		gEventRate[eEvent_ItemRate] = wEventRate*0.01f ;
		gEventRateFile[eEvent_ItemRate] = wEventRate*0.01f;
	}
	else if( wEventType == eEvent_MoneyRate )									// �Ӵ� ������� �����Ѵ�.
	{
...
{
	// 080118 LYW --- MapNetworkMsgParser : GM���� ó�� �� �̺�Ʈ ������ ������ üũ.
...
// 081022 LYW --- MapNetworkMsgParser : �� ����/��Ÿ �׽�Ʈ�� ���Ͽ� Pet���� GOD ��带 �����Ѵ�.
	...
	case MP_PARTY_INVITE_BYNAME_SYN:
		{
			...
			message.dwData5 = DWORD(party->GetDiceGradeOpt());
			SafeStrCpy(
				message.Name,
	...
	case MP_NPC_CHECK_HACK_SYN:
		{
			...
			// 081009 LYW --- MapNetworkMsgParser : �����ҽ� ����
			// 135 -- ����, 136 -- �罨
			// 081008 KTH -- ���� ���������� �ƹ��� Ŭ�� �� �� ����.
			...
			{
				// 081106 LYW --- MapNetworkMsgParser : ���� ���� ����ó�, 
				// ������ ������ ��尡 ������쿹�� ó���� �Ѵ�.
				switch( pmsg->dwData2 )
				{
				// ���� ���� ����
...
FIND_HACK:
	...
	// 090227 ShinJS --- �̵�NPC�� �̵�ó�� �߰�
	case MP_NPC_REMOVE_MOVENPC_SYN:
		{
			// �̵�NPC ���� �޼���
			MSG_WORD* pmsg = (MSG_WORD*)pMsg;		// (�̵�NPC Index)
	...
	case MP_NPC_MOVE_MOVENPC_SYN:
		{
			// �̵�NPC �̵������� ������� �̵�NPC �����ڿ� �̵�NPC�� �߰��ϰ�
			// �̵����� �޼����� �����Ͽ� ���� �Ҽ� �ֵ��� �Ѵ�
			MSG_WORD3* pmsg = (MSG_WORD3*)pMsg;		// (�̵����ʹ�ȣ, NPC Index, NPC MoveIndex)
			WORD wMoveNpcIdx = pmsg->wData2;									// �̵�NPC Index
			WORD wCurMoveIdx = pmsg->wData3;									// �̵�NPC �̵����� Index			
			...
			if( NPCMOVEMGR->AddMoveNpc( wMoveNpcIdx, wCurMoveIdx ) )
			{
				...
				msg.wData1 = pmsg->wData1;										// �̵��� �� ��ȣ
				msg.wData2 = wMoveNpcIdx;										// �̵�NPC Index

				g_Network.Send2AgentServer( (char*)&msg, sizeof(MSG_WORD2) );
			}
		}
		break;
	// 091014 ONS ����ġ���� NPC Ȱ��ȭ�� Ʈ���� �޼����� �߻���Ų��.
	...
	case MP_EMOTION_SITSTAND_SYN :
		{
			...
			if( pPlayer )
			{
				...
				// 080513 LYW --- MapNetworkMsgParser : ��� ó�� ����.
...
// 081126 LYW --- MapNetworkMsgParser : ����Ʈ ���� �߰�.
...
void Cheat_Fishing_FishPoint(char* pMsg)
{
	MSG_DWORDEX* pmsg = (MSG_DWORDEX*) pMsg;
	CPlayer* pPlayer = (CPlayer*)g_pUserTable->FindUser(pmsg->dwObjectID);
	pPlayer->SetFishPoint(pmsg->dweData);
	...
	case MP_NOTE_GETPACKAGE_SYN:
		{
			...
			if(dwItemDBIdx > 0)
			{
				...
				if(emptyCount == 0)
					dwErrorKind = eConsignmentERR_PoorSlot;	// �κ� ����� ����
			}
