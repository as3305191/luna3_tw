...
WORD CGeneralGridTable::FindMonsterGroupInRange(DWORD* pFriendsList, CObject* pFinder,GridPosition* pGridPosition,VECTOR3* pPos)
{
	CGrid* pGrid = GetGrid(pGridPosition->x,pGridPosition->z);
	if(pGrid == NULL)
		return 0;

	return pGrid->FindMonsterGroupInRange(pFriendsList, pFinder,pPos);
}
...
void CGeneralGridTable::AddObject(CObject* pObject, VECTOR3* pPos)
{
	...
	CGrid* pGrid = GetGrid( WORD( nx ),WORD( nz ) );
	if(pGrid == NULL)
		return;

	pGrid->AddObject(pObject);
	
	pObject->GridPos.x = WORD( nx );
	pObject->GridPos.z = WORD( nz );
}

void CGeneralGridTable::RemoveObject(CObject* pObject)
{
	CGrid* const pGrid = GetGrid(
		WORD(pObject->GetGridPosition()->x),
		WORD(pObject->GetGridPosition()->z));
...
void CGeneralGridTable::ChangeGrid(CObject* pObject, VECTOR3* pPos)
{
	...
	// 091026 LUJ, 진입하는 그리드 내의 오브젝트에게 알려준다
	{
		MSGBASE* message = 0;
		...
		PACKEDDATA_OBJ->Init(
			message,
			WORD(messageLength));
		AddGridToPackedDataExceptLastGrid(
			pObject->GetGridPosition(),
			pObject->GetID(),
			PACKEDDATA_OBJ);
		PACKEDDATA_OBJ->Send();
	}
	...
	// 091026 LUJ, 이전에 있던 그리드 내의 오브젝트에게 알려준다
	{
		MSGBASE* message = 0;
		...
		PACKEDDATA_OBJ->Init(
			message,
			WORD(messageLength));
		AddLastGridToPackedDataExceptCurrentGrid(
			pObject->GetGridPosition(),
			pObject->GetID(),
			PACKEDDATA_OBJ);
		PACKEDDATA_OBJ->Send();
...
{
	CGrid* pLastGrid = GetGrid( WORD( LastGridNumX), WORD( LastGridNumZ) );
	CGrid* pCurGrid = GetGrid( WORD( GridNumX ), WORD( GridNumZ ) );
	if(pCurGrid)
