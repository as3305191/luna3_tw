...
//	STARTLOG(CONDITIONMGR->LoadConditionList());
	...
	PartyLoad(0);
	GuildLoadGuild(0);
	// 090316 LUJ, ���� ���۵� �ʿ����� �ڵ� ��ȯ/ž������ �ʵ��� �ʱ�ȭ�Ѵ�
	ResetVehicle( GetMapNum() );
	GuildResetWarehouse( m_wMapNum );
	// 100305 ONS �ʱ⵿�� DB�� PC�� ���������� �ʱ�ȭ�Ѵ�.
	PCROOMMGR->Init();
...
void CServerSystem::Process()
{
	...
	while( (pObject = (CObject*)g_pUserTable->GetUserData()) != NULL )
	{
		...
		{
			// EndObjectState() �ȿ��� CObject�� �����Ѵ�! ��� ������ ��� ������ �޸𸮸� �׼����ϰ� �ȴ�
...
CNpc* CServerSystem::AddNpc(BASEOBJECT_INFO* pBaseObjectInfo,NPC_TOTALINFO* pTotalInfo,VECTOR3* pPos)
{
	...
	// 081012 LYW --- ServerSystem : npc null üũ�� �Ѵ�.
	if( !pNpc ) return NULL ;
...
{	
	...
	if(HOUSINGMAPNUM == GetMapNum())
	{
		...
		! pPlayer->IsDungeonObserver())	// ������������ �ο�üũ ���� �ʴ´�.
	{
		--playerSizeInChannel;
	}
	...
	else if(SIEGEWARFAREMGR->IsSiegeWarfareZone(g_pServerSystem->GetMapNum(), FALSE) || SIEGEDUNGEONMGR->IsSiegeDungeon(g_pServerSystem->GetMapNum()))
	{
...
ULONGLONG CServerSystem::GetGameRoomKey(LPCTSTR address) const
{
	...
	// 091228 LUJ, ���ڿ��� ��ġ�� �����Ѵ�(��: 192.168.1.130 -> 192168001130)
