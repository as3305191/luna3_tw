...
void InsertLogExp( BYTE bType, DWORD charIdx, LEVELTYPE level, EXPTYPE changeValue, EXPTYPE NowExp, WORD MurdererKind, DWORD MurdererIdx, DWORD CurAbilPoint)
{
	...
	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC dbo.UP_EXPPOINTLOG %d, %d, %d, %I64u, %I64u, %d, %d, %d",
		bType,
		charIdx,
		level,
		changeValue,
...
void InsertLogMoney( BYTE type, DWORD charIdx, MONEYTYPE InvenMoney, MONEYTYPE ChangeValue, DWORD TargetID, MONEYTYPE TargetMoney )
{
	...
	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC dbo.UP_MONEYLOG %d, %d, %I64u, %I64u, %d, %I64u",	
		type,
		charIdx,
		InvenMoney,
		ChangeValue,
				  ...
				  DURTYPE ItemDur, EXPTYPE ExpPoint)
{
	...
	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC dbo.UP_ITEMMONEYLOG %d, \'%s\', %d, \'%s\', %d, %I64u, %I64u, %I64u, %d, %d, %d, %d, %u, %I64u",
		FromChrIdx,
		FromChrName,
		ToChrIdx,
		ToChrName, 
...
void LogCheat(DWORD Character_Idx, BYTE LogType, MONEYTYPE Param, DWORD ItemIdx, DWORD ItemDBIdx)
{
	...
	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC dbo.UP_CHEATE_LOG %d, %d, %I64u, %d, %d",
		Character_Idx,
		LogType,
		Param,
		ItemIdx,
...
void LogGuild(DWORD CharacterIdx, DWORD GuildIdx, WORD LogType, DWORD Param1, DWORDEX Param2)
{
	...
	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC dbo.UP_GUILDLOG %d, %d, %d, %d, %I64u",
		CharacterIdx,
		GuildIdx,
		LogType,
		Param1,
...
{
	// 081012 LUJ, �Ϻ� ������ ���ν��� ȣ�� �� ��ҹ��ڱ��� ��ġ�ؾ� �Ѵ�. ��, dbo�� �ҹ��ڷ� �ؾ���. �̿� ��� ���ν��� �̸��� ������
	sprintf(txt, "EXEC  %s %d, %I64u, %d, %I64u, %d, %d",	"dbo.MP_CHARACTER_UPDATEINFOBYTIME", 
...
void InsertReversedTransmogItem(DWORD dwUserIdx , DWORD ItemIdx)
{
	g_DB.FreeMiddleQuery(
		RInsertReversedTransmogItem,
		0,
		"EXEC dbo.MP_ITEM_INSERT_REVERSED_TRANSMOG %d, %d, %d, %d",
		dwUserIdx,
		ItemIdx,
		1,
		0,
...
{
	const MIDDLEQUERYST& r = query[ 0 ]; 

	if(dbMessage->dwResult > 0)
	{
		LogItemMoney(
					0,
					"*reverseTransmog",
					_atoi64((char*)r.Data[3]),
					"*itemmall",
					eLog_ItemTransmogReversed, 
					0,
					0,
					0,
					_atoi64((char*)r.Data[2]),
					_atoi64((char*)r.Data[0]),
					0,
					_atoi64((char*)r.Data[1]),
					1,
					0 );
	}
}

void ItemTransmogInsert( const ITEM_OPTION& option, DWORD dummyDBIdx, DWORD CharacterIdx)
{
	// 081012 LUJ, ï¿½Ýºï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½Î½ï¿½ï¿½ï¿½ È£ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½Ò¹ï¿½ï¿½Ú±ï¿½ï¿½ï¿½ ï¿½ï¿½Ä¡ï¿½Ø¾ï¿½ ï¿½Ñ´ï¿½. ï¿½ï¿½, dboï¿½ï¿½ ï¿½Ò¹ï¿½ï¿½Ú·ï¿½ ï¿½Ø¾ï¿½ï¿½ï¿½. ï¿½Ì¿ï¿½ ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½Î½ï¿½ï¿½ï¿½ ï¿½Ì¸ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
	g_DB.FreeMiddleQuery(
		0,
		CharacterIdx,
		"EXEC dbo.MP_ITEM_TRANSMOG %d, %d, %d",
		option.mItemDbIndex,
		dummyDBIdx,	 
		option.mTransmogIdx
		 );
...
void ItemTransmogReverse( const ITEM_OPTION& option)
{
	...
	g_DB.FreeMiddleQuery(
		0,
		0,
		"EXEC dbo.MP_ITEM_TRANSMOG_REVERSE %d",
		option.mItemDbIndex
		 );
}
...
void RCharacterStorageItemInfo(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	{	
		...
		item.Position = AdjustItemPositionForCardSlots( item.Position, ItemInfo, false );
...
void RQuestItemLoad(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	// client¡Ë?¡Ë¢ç ¡§¡þ¡Ë¡þ¨Ý©ª¡§u ¢®¨ú¡Ë¡þvA¡§u ¢®iy¡§u¡§¡þ..
	msg.Category = MP_QUEST;
...
void RLoadGuildWar( LPQUERY pData, LPDBMESSAGE pMessage )
{
	...
	for( DWORD i = 0; i < pMessage->dwResult; ++i )
	{
		...
		DWORDEX dwMoney = (DWORDEX)_atoi64((char*)pData[i].Data[3]);
...
void RGuildAddSkill( LPMIDDLEQUERY query, LPDBMESSAGE dbMessage )
{
	...
	const MONEYTYPE	money		= MONEYTYPE(_atoi64( ( char* )query[ 0 ].Data[ 2 ] ) );
...
void FishingData_Update(DWORD dwPlayerID, WORD wFishingLevel, EXPTYPE dwFishingExp, DWORDEX dwFishPoint)
{
	...
	g_DB.FreeMiddleQuery(
		NULL,
		dwPlayerID,
		"EXEC  dbo.MP_FISHINGDATA_UPDATE %d, %d, %0.f, %I64u",
...
void Log_Fishing(DWORD dwPlayerID, CHAR cLogType, DWORDEX dwDiffPoint, DWORDEX dwTotalPoint, DWORD dwBuyItemIdx, DWORD dwUseFish, EXPTYPE dwExp, DWORD dwLevel )
{
	...
	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC dbo.UP_FISHING_LOG_INSERT %d, %d, %I64u, %I64u, %d, %d, %0.0f, %d",
		dwPlayerID,
		cLogType,
		dwDiffPoint,
		dwTotalPoint,
...
void RGTRewardMoneyInsert(LPQUERY pData, LPDBMESSAGE pMessage)
{
	...
	DWORDEX dwMoney = (DWORDEX)_atoi64((char*)pData->Data[1]);
...
void ROption_Load(LPMIDDLEQUERY query, LPDBMESSAGE dbMessage)
{
	...
	pPlayer->GetCharacterTotalInfo().bWingsMove = BOOL(atoi((char*)record.Data[1]));
	pPlayer->SetInitState(
...
void Option_Update(CPlayer& pPlayer)
{
	g_DB.FreeMiddleQuery(
		NULL,
		pPlayer.GetID(),
		"EXEC dbo.MP_OPTIONUPDATE %d, %d, %d",
		pPlayer.GetID(),
		pPlayer.GetCharacterTotalInfo().HideFlag,
		pPlayer.GetCharacterTotalInfo().bWingsMove);
}
...
void UpdatePlayerOnMapOutWithOutMoney( DWORD playerIndex, DWORD userIndex, DWORD mapIndex, WORD channelIndex )
{
	g_DB.FreeMiddleQuery(
		...
		playerIndex,
		// 081219 LUJ, ������ ��
		"EXEC dbo.MP_CHARACTER_UPDATE_ON_MAPOUT_WITHOUT_MONEY %u, %u, %u, %d",
		playerIndex,
		userIndex,
		mapIndex,
		channelIndex );
...
void RConsignment_Regist(LPMIDDLEQUERY query, LPDBMESSAGE dbMessage)
{
	...
	if(nError < 0)
	{
		...
		msg.dwData4 = (DWORD)atoi( ( char* )query[0].Data[3] );	// �Ҹ�Ⱥ��
...
void Consignment_Log(DWORD dwConsignmentIdx, DWORD dwPlayerID, int nKind, DWORD dwDBItemIdx, DWORD dwItemIndex, DWORD dwChangeDurability, DWORDEX dwPrice)
{
	g_DB.LogMiddleQuery(
		NULL,
		dwPlayerID,
		"EXEC dbo.TP_CONSIGNMENT_LOG %d, %d, %d, %d, %d, %d, %I64u",
...
{
	sprintf( txt, "EXEC dbo.MP_SKILLTREE_SkillMoney_Info %d", dwPlayerID );
	g_DB.Query(eQueryType_FreeQuery, eSkillTree_Money_Load, dwPlayerID, txt, dwPlayerID);
}
...
void RSkillTree_Money_Load(LPQUERY pData, LPDBMESSAGE dbMessage)
{
	...
	DWORDEX dwTotalSkillMoney = DWORDEX(atoi((char*)pData->Data[0]));
...
void SkillMoneyUpdate( DWORD dwPlayerID, DWORDEX addskillmoney )
{
	g_DB.FreeMiddleQuery(
		0,
		0,
		"EXEC dbo.MP_SKILLTREE_SkillMoney_Update %d, %I64u",
		dwPlayerID,
		addskillmoney
		);
}
...
void SkillCastingHack_Log(CPlayer* pPlayer, DWORD dwTypeHack)
{
	...
	char buf1[66] = { 0 };
	if (wType == 1)
		wsprintf(buf1, "CoolDownHack");
	else if (wType == 2)
		wsprintf(buf1, "AreaHack");
	else if (wType == 3)
		wsprintf(buf1, "RangeHack");
	else
		wsprintf(buf1, "Unknown");

	g_DB.LogMiddleQuery(
		0,
		0,
		"EXEC luna_logdb.dbo.TP_SKILL_HACK_LOG_INSERT %d, '%s'",
		pPlayer->GetID(),
		buf1);
}