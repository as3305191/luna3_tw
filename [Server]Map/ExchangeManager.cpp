	...
	case MP_EXCHANGE_ADDITEM_SYN:
		{
			...
			// 071125 KTH --- ë´‰ì¸???€ë¦??„ì´?œì? êµí™˜??ë¶ˆê??¥í•˜?? 080204 KTH -- êµí™˜ ë¶ˆê??¥í•œ ?„ì´?œë„ êµí™˜??ë¶ˆê??¥í•˜??
			...
			{
				// 091215 ONS ?„ì´?œStackìµœë??˜ë? ?„ì´?œë³„ë¡?ì§€?•ëœ ê°’ìœ¼ë¡?ì²˜ë¦¬?œë‹¤.
	...
	case MP_EXCHANGE_INPUTMONEY_SYN:
		{
			...
			//KES
			if( pRoom->IsLocked( pPlayer1->GetExchangeContainer()->nMyIndex ) || pRoom->IsLocked( pPlayer2->GetExchangeContainer()->nMyIndex )
				|| pRoom->GetExchangeMoney( pPlayer1->GetExchangeContainer()->nMyIndex ) > 0 )  //?œìª½?´ë¼???½ì„ ?ˆê±°?? ?ˆì„ ?œë²ˆ?´ë¼???…ê¸ˆ?ˆì„ ê²½ìš°
			{
				MSGBASE NackMsg;
