...
private:
	char m_Language[50]; 
	std::map<std::string, std::string> m_TranslationsPath; 
	std::map<std::string, std::string> m_Translations;
    std::map<DWORD, std::string> m_ItemTranslations;
    std::map<DWORD, std::string> m_SkillTranslations;
    std::map<DWORD, std::string> m_MonsterTranslations;
    std::map<DWORD, std::string> m_NpcTranslations;
    std::map<DWORD, std::string> m_BuffTranslations;
...
public:
	...
	MONEYTYPE GetTransmogCost(const ITEM_INFO&) const;
	...
	void GetMonsterStats(WORD wMonsterKind, monster_stats * stats);				// ¸ó½ºÅÝ °´Ã¼ ÃÊ±âÈ­µÉ¶§
	BOOL LoadNpcChxList();
	...
	WORD CheckMapChangeForMapID(VECTOR3* pVector, WORD curMapNum);
...
private:
	...
	CYHHashTable<stDungeonKey>		m_DungeonKeyList;	// 인띘 키 리스트
...
public:
	...
	// 090227 ShinJS --- 암생인추가작업 (이띙NPC 정보 별띄생성:[서버:맵엝 로그인시 활용], [흴라이언트:이띙NPC정보 파악])
private:
	CYHHashTable<STATIC_NPCINFO>	m_htMoveNpcStaticInfo;										// 이띙NPC의 StaticNPC 정보
	void LoadMoveNPCInfo();																		// StaticNpc.bin엝서 이띙NPC 정보만 저장한다
public:	
	STATIC_NPCINFO* GetMoveNpcInfo( WORD wNpcUniqueIdx ) { return (STATIC_NPCINFO*)m_htMoveNpcStaticInfo.GetData( wNpcUniqueIdx ); }

	// 091106 LUJ, 리미트 띘전 정보 관리
public:
...
private:
	typedef std::pair<DWORD, BYTE>									DualWeaponModListKey;			// modfile인띱스와 종족정보를 키로 사용한다.	
	typedef std::map<DualWeaponModListKey, std::string>				DualWeaponModListMap;			 
	DualWeaponModListMap											m_DualWeaponModList;			 		
public:
	void ParseModFileName( char* pInString, char* pRObjectName, DWORD wIndex, BYTE byRaceType, BYTE byGender );		// '/'로 구분지어진 양솝무기 modfile명을 파싱한다.
	const char* GetDualWeaponLObject( WORD wIndex, BYTE byRaceType, BYTE byGender );										// 인띱스, 종족, 성별정보를 통해서 해당 modfile명을 반환한다.


	// 100225 ShinJS --- 캝릭터들의 초기 스탯 정보를 가져온다.
private:
...
//-----------------------------------------------------------------------------------------------------------//
    ...
    void LoadLanguage();
    char* GetLanguage() { return m_Language; }
    const char* GetTranslationPath(const char* file, eLanguageBin bin = WHOLELIST); 
    const char* GetTranslation(eLanguageBin bin, DWORD index, const char* str) const ;
...
//	void GetMonsterStats(WORD wMonsterKind, MONSTER_STATS * stats);
	...
	// 090603 LUJ, 부활 지젝이 없띔라띄 종료띘지 않띄록, 기본값을 반환하띄록 함
...
//----------------------------
	...
	// 090227 ShinJS --- 암생인추가작업 (StaticNPC를 추가/삭제할수 있띄록 함, 암생인 추가정보 별띄생성:맵엝 로그인시 활용)
public:
	CYHHashTable<STATIC_NPCINFO>* GetMoveNpcInfoTable() { return &m_htMoveNpcStaticInfo; }
	void AddMoveNpcInfo( WORD wNpcIdx, WORD wNpcKindIdx, WORD wXPos, WORD wZPos, WORD wDir );	// 이띙NPC 추가
	void RemoveMoveNpcInfo( WORD wNpcUniqueIdx );												// 이띙NPC 제거
