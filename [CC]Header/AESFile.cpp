...
cAESFile::cAESFile()
{
	...
	m_dwLastSecretKey = dwSecretKey[m_nVersionCount-1];
...
BOOL cAESFile::SetBinVersion( DWORD	BinVersion )
{
	...
	{	
		if(dwVersion[i] == BinVersion)
		{
			...
			m_dwCurSecretKey = dwSecretKey[i];
...
BOOL cAESFile::AESEncryptData(FILE* fp, char* pFileName, char* pData, DWORD dwDataSize)
{
	...
	header.dwSecretKey = m_dwLastSecretKey;
	if( header.dwDataSize == 0 )
		header.dwType = 1;
	else
		header.dwType = rand()%header.dwDataSize + 1;
	header.dwVersion += ((header.dwSecretKey*2)+header.dwType+header.dwDataSize);
