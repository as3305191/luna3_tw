...
BOOL CPackExtractDlg::OnInitDialog()
{
	...
	m_pStorage->Initialize( MAX_FILE_ATOM_NUM, 4096, 128, 20250813, FILE_ACCESS_METHOD_ONLY_FILE );
...
void CPackExtractDlg::ExtractPackFile()
{
	...
	for( iter = m_vPackFile.begin() ; iter != m_vPackFile.end() ; ++iter )
	{
		...
		pPackFileHandle = m_pStorage->MapPackFile( (LPSTR)(LPCSTR)strPackFile, 20250813, TRUE );
		if (!pPackFileHandle)
			break;
...
void CPackExtractDlg::OpenExtractPackFileWithSelectListDlg( CString strSelectedPackFile )
{
	...
	I4DyuchiFileStorage* fileStorageSub = NULL;
	HMODULE hStorage = LoadLibrary(  _T("FileStorage2.dll") );
	if( hStorage )
	{
		CREATE_INSTANCE_FUNC	pFunc;
		pFunc = (CREATE_INSTANCE_FUNC)GetProcAddress(hStorage,"DllCreateInstance");
		HRESULT hr = pFunc((void**)&fileStorageSub);
		if (hr != S_OK)
		{
	...
	fileStorageSub->Initialize( MAX_FILE_ATOM_NUM, 4096, 128, 20250813, FILE_ACCESS_METHOD_ONLY_FILE );

	m_pSelectedPackFileHandle = fileStorageSub->MapPackFile( (LPSTR)(LPCSTR)strPackFile, 20220406, TRUE );
	...
	FSFILE_ATOM_INFO*	pFileInfoList;
	DWORD dwFileInfoNum;
	fileStorageSub->GetPackFileInfo( m_pSelectedPackFileHandle, &fileInfo );
	if( m_pSelectedPackFileHandle )
	{
		dwFileInfoNum = fileStorageSub->CreateFileInfoList( m_pSelectedPackFileHandle, &pFileInfoList, MAX_FILE_ATOM_NUM );
...
void CPackExtractDlg::CompressFile( CString strFolderName )
{
	...
	void*	pPackFileHandle = NULL;
	pPackFileHandle = m_pStorage->MapPackFile( (LPSTR)(LPCSTR)strPackName, 20250813, TRUE );
	if (!pPackFileHandle)
	{
		m_pStorage->UnmapPackFile( pPackFileHandle );
		return;
