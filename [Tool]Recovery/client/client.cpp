...
void CclientApp::LoadLicenseScript()
{
	...
	while( ! file.IsEOF() )
	{
		...
		mLicenseFavoriteMap.insert( std::make_pair( index, CString( file.GetString() ) ) );
	}
	...
	while( ! file.IsEOF() )
	{
		...
		mLicenseAreaMap.insert( std::make_pair( index, CString( file.GetString() ) ) );
	}
...
void CclientApp::LoadTip()
{
	...
	while( ! file.IsEOF() )
	{
		...
		if( ! _strcmpi( "#Msg", keyword ) )
		{
			...
			mSkillTipMap.insert( std::make_pair( index, CString( buffer ) ) );
