...
void CPartyIconManager::CreatePartyIcon(DWORD ID, PARTY_MEMBER* pMember)
{
	...
	for(i=0; i<MAX_PARTY_LISTNUM-1; i++)
	{
		...
		{
			const VECTOR3& position = pMember->mPosition;
			GAMEIN->GetMiniMapDialog()->SetPartyIconObjectPos(
				ID,
				int(position.x),
				int(position.z));
			m_MoveInfo[i].SetPosition(
				int(position.x),
				int(position.z));
			m_MoveInfo[i].SetIconPosition(
				int(position.x),
				int(position.z));
			return;
		}
	...
	// 100105 NYJ - 맵이 다르면 추가할 필요없다.
	if(pMember->mMapType != MAP->GetMapNum())
		return;
