...
void CCharacterDialog::UpdateData()
{
	...
	// 090504 ONS ?ùùù?ùù?ù?ù ù?ùùùùùùù ù?ù Ûùù
	...
	// 080214 LYW --- CharacterDialog : ù??ù ùùùù ùùù?ù? ù?ù.
	RefreshFamilyInfo() ;
}
...
{
	char buf1[66] = {0, } ;
	wsprintf( buf1, "%d", HERO->DoGetAccuracyRate() ) ;
...
{
	char buf1[66] = {0, } ;
	wsprintf( buf1, "%d", HERO->DoGetEvasionRate() ) ;
...
void CCharacterDialog::RefreshFamilyInfo()
{
	...
	if( pFamily )																	// ù??ù ùùùùùù ùù?ùùùù ¸?ù?ù.
	{
		m_pFamilyName->SetStaticText(pFamily->Get()->szName) ;						// ù??ùùùùù ùùùùù?ù.
	}
	else
	{
...
void CCharacterDialog::PrintRangeRate()
{
	char buf1[66] = {0, } ;
	wsprintf( buf1, "%d", HERO->DoGetRangeRate() );
...
{
	char buf1[66] = {0, } ;
	const float nBlockRate = HERO->DoGetBlockRate();
	if( nBlockRate > 0 )		sprintf( buf1, "+%0.f%%", nBlockRate );
	else						sprintf( buf1, "%0.f%%", nBlockRate );
...
{
	char buf1[66] = {0, } ; 
	const DWORD nBlockRate = HERO->DoGetCriticalDamage();
	if( nBlockRate > 0 )		sprintf( buf1, "+%d%%", nBlockRate );
	else						sprintf( buf1, "%d%%", nBlockRate );
...
void CCharacterDialog::CheckHeroImage()
{
	...
	// 090423 ONS ù?ùùùùù ?ùùùù ùùùù ùùù?ù?ù ùùùùù ù?ùùù ù?ù
	for( count = 0 ; count < MAX_CHAR_IMAGE ; ++count )
	{
