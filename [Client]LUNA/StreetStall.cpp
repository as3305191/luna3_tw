...
void CStreetStall::DeleteItem( ITEMBASE* pbase )
{
    ...
    for(i = 0; i < m_pStallGrid->GetCellNum(); ++i)
    {
        ...
        if (pItem == NULL) 
        {
            // ASSERTMSG(0, "Server info not found in client...");
            continue;
        }
    ...
    if (m_pStallGrid->DeleteIcon(pos, &pIcon))
    {
        ...
        {
            m_pStallGrid->DeleteIcon(j + 1, &pIcon);
            if (!pIcon) break;
...
void CStreetStall::RegistMoney( POSTYPE pos, DWORDEX dwMoney )
{
...
void CStreetStall::ChangeItemStatus( POSTYPE pos, DWORDEX money, BOOL bLock )
{
	CExchangeItem * pItem = (CExchangeItem*)m_pStallGrid->GetIconForIdx( pos );
	if( !pItem ) return;
	char buf[128] = {0,};
...
void CStreetStall::ChangeItemStatus( ITEMBASE* pbase, DWORDEX money, BOOL bLock, int nDivideKind )
{
...
// 매니져에 영약 아이템 사기 호출 ..
	STREETSTALLMGR->BuyItemDur( pos, (DURTYPE)param1 );
