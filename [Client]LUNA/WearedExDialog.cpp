...
{	
	...
	pHero->SetWearedItemTransmog(relPos, ITEMMGR->GetOption(item->GetDBIdx()).mTransmogIdx);
...
{
	CHero* pHero = OBJECTMGR->GetHero();
	//KES 031128 ¹«°ø¾ÆÀÌÅÛ ÀåÂø°¡´É ÅøÆÝ º¯°æ
	...
	pHero->SetWearedItemIdx(relPos, 0);
	    pHero->SetWearedItemTransmog(relPos, 0);
	pHero->SetCurComboNum(SKILL_COMBO_NUM);
