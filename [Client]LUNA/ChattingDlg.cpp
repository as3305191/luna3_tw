...
void CChattingDlg::AddShoutMsg(char* msg, DWORD dwColor, BOOL bColorfulMegaphone, DWORD dwBackColor)
{
	char buf[ MAX_PATH ] = {0};
	...
	if(strlen(buf) > 0)
	{
		m_pShoutList->AddItem(buf, dwColor, -1, 0, 0, bColorfulMegaphone, dwBackColor);
	}

	if( nLen > 0 )
	{
		m_pShoutList->AddItem(msg, dwColor, -1, 0, 0, bColorfulMegaphone, dwBackColor);
	}
}
