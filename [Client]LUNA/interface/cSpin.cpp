...
DWORD cSpin::ActionEvent(CMouse * mouseInfo)
{
	...
	// 080710 LYW --- cSpin : 스핀 다이얼로그 상의 버튼에서 발생한 이벤트 반환을 하지 않아서,
	// 스핀 다이얼로그 아래에 있는 다른 컨트롤들이 있으면, 스핀 버튼 처리 후, 
	// 다른 컨트롤 이벤트 까지 처리가 되므로 이를 수정 함.
	we |= m_upBtn->ActionEvent(mouseInfo) ;
    if(we & WE_BTNCLICK)
	{
		(*cbWindowFunc)(m_ID, m_pParent, WE_SPINBTNUP);
	}

	we |= m_downBtn->ActionEvent(mouseInfo) ;
	if(we & WE_BTNCLICK)
	{
		(*cbWindowFunc)(m_ID, m_pParent, WE_SPINBTNDOWN);
	}
