...
void cIconDialog::SetHide(BOOL val)		
{
	if( m_bDisable ) return;
	...
	{
		if(m_pIconCell[i].use == USE && m_pIconCell[i].icon)
			m_pIconCell[i].icon->SetHide(val);
	}
}
...
{
	for(WORD i = 0 ; i < m_wCellNum ; ++i)
	{
		const RECT& cell = m_pIconCell[ i ].rel_rect;

		if( m_absPos.x + cell.left < mouseX		&& 
			m_absPos.y + cell.top < mouseY		&&
			m_absPos.x + cell.right > mouseX	&&
			m_absPos.y + cell.bottom > mouseY )
		{
...
BOOL cIconDialog::GetIconCellRect(WORD wCell, RECT& rt)
{
	if(m_wCellNum <= wCell)
		return FALSE;

	rt = m_pIconCell[wCell].rel_rect;
	return TRUE;
}