...
#include "../StallKindSelectDlg.h"		// LYJ 051017 ���Գ����� �߰�
...
#include "../ReinforceGuideDialog.h"		// 06. 02. ��ȭ ��� �ȳ� �������̽� �߰� - �̿���
...
#include "cScrollIconGridDialog.h"		//090330 pdy ScrollIconGridDialog �߰�
...
#include "UiSettingManager.h"			// 0911116 ONS UI��ġ���� 
...
// 080112 LYW --- cScriptManager : ���̾�α� ���̺� enum �ڵ� �߰� ó��.
enum ENUM_DLGINFO_TABLES
{
	...
	// desc_hseos_����Ʈ ��_01
	// S ����Ʈ �� �߰� added by hseos 2008.01.24
	eCHALLENGEZONECLEARNO1DLG,
	// E ����Ʈ �� �߰� added by hseos 2008.01.24
	eCHANGENAMEDLG, 
	eNUMBERPAD_DLG,
	...
	eFISHINGDLG,		// 080421 NYJ --- ���� ���̾�α�
	eFISHINGGAUGEDLG,	// 080422 NYJ --- ���ð����� ���̾�α�
	eFISHINGPOINTDLG,	// 080505 NYJ --- ��������Ʈ ���̾�α�

	eWEATHERDLG,		// 080505 NYJ --- ���� ���̾�α�
...
// 080401 LYW --- cScriptManager : Add dialogs part for chatroom system.
	...
	// 080929 LYW --- cScriptManager : ���� ��� ���� ���̾�α� �߰�.
	eSIEGEWARFALGDLG,

	// 080930 LYW --- cScriptManager : ���̵� ��/�ƿ� ���̾�α� �߰�.
	eFADEDLG,
	ePOPUPMENUDLG,
...
char	g_szDlgInfoTable[][50]	=	
{
	...
	"$GDTENTRYDLG",						// 86 �����ʸ�Ʈ DLG
	"$GDTENTRYCANCELDLG",				// 87 �����ʸ�Ʈ DLG
	"$GTBATTLETABLE_8",					// 88 �����ʸ�Ʈ DLG
	"$GDTVIEWDLG",						// 89 �����ʸ�Ʈ DLG (��Ʋ����Ʈ)
	"$GTSCORETIMEDLG",					// 90 �����ʸ�Ʈ DLG
	...
	"$GUILDREVIVALDLG",					// 129
	// desc_hseos_����ý���_01
	// S ����ý��� �߰� added by hseos 2007.08.24	2007.09.10
	"$FARM_BUY_DLG",					// 130
	"$FARM_UPGRADE_DLG",				// 131
	// E ����ý��� �߰� added by hseos 2007.08.24	2007.09.10
	"$PROGRESSDLG",						// 132

	// desc_hseos_����ý���_01
	// S ����ý��� �߰� added by hseos 2007.10.23
	"$FARM_MANAGE_DLG",					// 133
	// E ����ý��� �߰� added by hseos 2007.10.23
	...
	// 071227 LYW --- cScriptManager : ĳ���� �̸� ���� ���̾�α� �߰�.
	"$CHANGENAMEDLG",					// 143

	// 080109 LYW --- cScriptManager : ���� �е� ���̾�α� �߰�.
	"$NUMBERPAD_DLG",					// 144
...
char	g_szPropertyInfoTable[][50]	=	
{
	...
	//090330 pdy ScrollIconGridDialog �߰�
	"#VIEW_COLS",
	"#VIEW_ROWS",

	// 090820 ShinJS List Dialog ���� �߰�
	"#SORT_LIST",

	// 091125 ShinJS --- List Dialog, Ŭ��Item�� ���Ͽ� ���콺 OverImage Render ���� �߰�
	"#REMAIN_OVERIMAGE",

	// 091116 ONS UI��ġ ���� �Ӽ� �߰�
	"#SAVE_POSITION",
	// 091207 ONS UI���� Ȱ��ȭ���� �Ӽ� �߰�
	"#SAVE_ACTIVITY",

	// 091209 ShinJS --- List Dialog, TopListItem ����� Func ���� ���� �߰�
	"#USE_TOPROW_CHANGE",
	"#OPEN_SOUND",
	"#CLOSE_SOUND",
...
cWindow * cScriptManager::GetDlgInfoFromFile(char * filePath, char* mode)
{
	...
	while(1)
	{
		...
		case eQUESTQUICKVIEWDLG :	// "$QUESTQUICKVIEWDLG											// ����Ʈ �˸��� ���̾�α׸� ���� ��Ʈ.
			if((fp.GetString())[0] == '{')															// �о���� ��Ʈ���� { ���,
			{
				dlg = (cDialog*)GetInfoFromFile((cWindow*)(new CQuestQuickViewDialog), &fp) ;		// ����Ʈ �˸��� ���̾�α׸� �����Ͽ� �����츦 �ʱ�ȭ �Ѵ�.
			}
			else
				_asm int 3 ;
			break ;
		...
		case eTRANSMOGDLG:
			if((fp.GetString())[0] == '{')
				dlg = (CTransmogDialog*)GetInfoFromFile(new CTransmogDialog, &fp);
			else
				__asm int 3;
			break;
...
//KES 030821 �߰�
			case 22:			// $DLG")
				window = GetInfoFromFile((cWindow*)(new cDialog), fp);				// 293�����̶� ���� �ڵ� ��ġ�µ�.? ���ϸ� �� �Լ��� �� �Լ��� ��ĥ ���� ������ �ѵ�....
				wnd->Add(window);
				break;
	//SW051112 ���ָ��
...
{
	// 080114 LYW --- cScriptManager : Dialog �ؽ� ���̺��� ������� �ʵ��� ó�� ��.
	//// GetDlgInfoFromFile()�� �ؽ�.
	...
	//DWORD	dwDlgInfoTableNum	=	dwDlgInfoTableSize / 50;				// 50�� char[50].
	...
	DWORD	dwCtrlInfoTableNum	=	dwCtrlInfoTableSize / 50;				// 50�� char[50].
	...
	DWORD	dwPropertyInfoTableNum	=	dwPropertyInfoTableSize / 50;				// 50�� char[50].
...
//
	...
	//KES 030916
				...
				case 66:	// "#INITCOMBOLIST")
					{
						...
						GetImage(fp, &TopImage);					// ��� ��׶��� �̹���
						topHei = fp->GetWord();						// ��� �̹��� ���̰�.
						GetImage(fp, &MiddleImage);					// �߰� ��׶��� �̹���.
						middleHei = fp->GetWord();					// �߰� ���̰�(?)
						GetImage(fp, &DownImage);					// �ϴ�
						downHei = fp->GetWord();					// �ϴ� ����.
						GetImage(fp, &ListOverImage);				// Ŀ�� �÷����� �� ���ϴ� �̹���
				...
				case 74:	// "#SPINMINMAX")
					minValue = fp->GetLongLong();
					maxValue = fp->GetLongLong();
					break;
				case 75:	// "#SPINUNIT")
					spinUnit = fp->GetLong();
					break;
		...
		//			win->SetPieceImage(&GuagePieceImage);
				...
				// E �ػ� �߰� added by hseos 2007.06.12
					{
						...
						if (dispInfo.dwWidth == 800)
							wnd->cWindow::SetDeltaPos(deltaPosition800);
						else
							wnd->cWindow::SetDeltaPos(deltaPosition1024);
					}
					break;
				...
				// 071210 LYW --- cScriptManager : �� ���̾�α��� ��Ÿ���� �����ϴ� ���� �߰�.
				case 121 :	//"#TAB_STYLE"
					{
	...
	CASE(WT_SPIN)
		...
		// 090819 ShinJS --- cSpin �� Caret Color ���� �߰�
	...
	CASE(WT_COMBOBOX)
		...
		win->SetOverImageScale(ListScale.x, ListScale.y); // over image �߰�
	...
	CASE(WT_TEXTAREA)
		...
		win->InitTextArea(&textRect, nLimitBytes, &TopImage, topHei, &MiddleImage, middleHei, &DownImage, downHei );
 		win->SetLimitLine( limitLineNum );	//inittextarea���� �Ʒ��� �־���Ѵ�.
		win->SetReadOnly(bReadOnly);
	...
	//090323 pdy SCROLLICONGRIDDLG �߰� 
