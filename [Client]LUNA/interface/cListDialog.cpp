	...
	if(itemCnt > 0)
	{
		if(m_pGuageBar && IsShowScrool())
			if(m_pGuageBar->IsDrag() )
			{
				...
				// 091209 ShinJS --- TopListItemIdx ���� Event ���� Func ����
...
void cListDialog::ListMouseCheck( CMouse * mouseInfo )
{
	...
	if( selIdx < m_lLineNum && 0 <= selIdx )
	{
		...
		{
			// 080417 LUJ, Ȱ�� ������ ��쿡�� ������ ������ �ǵ��� ��
			// 090902 ShinJS --- Mouse Down �� ������ ���� ��쿡�� ���õǵ��� �Ѵ�
			if(		mouseInfo->LButtonDown() &&
				!	IsDisable() &&
...
void cListDialog::RenderNormalList()													// �⺻ ������� �����ϴ� �Լ�.
{
	...
	if( m_bStyle != LDS_SERVERLIST )
		...
		// 091125 ShinJS --- ���õ� Item�� ���Ͽ� ���콺 OverImage Render
				...
				if(CFONT_OBJ->IsCreateFont(m_wFontIdx))
				{
					...
					*/
					if( m_bStyle == LDS_NORMAL )
					{
						...
						{
							if(item->bColorful)
									CFONT_OBJ->RenderNoticeMsg(m_wFontIdx, item->string, lstrlen(item->string), &rect, RGBA_MERGE(0xffffff00, m_alpha * m_dwOptionAlpha / 100), RGBA_MERGE(item->backColor,180));
								else
									CFONT_OBJ->RenderFont(m_wFontIdx, item->string, lstrlen(item->string), &rect, RGBA_MERGE(0xffffff00, m_alpha * m_dwOptionAlpha / 100));
						}
						else
						{
							if(item->bColorful)
								CFONT_OBJ->RenderNoticeMsg(m_wFontIdx, item->string, lstrlen(item->string), &rect, RGBA_MERGE(item->rgb, m_alpha * m_dwOptionAlpha / 100), RGBA_MERGE(item->backColor,180));
							else
								CFONT_OBJ->RenderFont(m_wFontIdx, item->string, lstrlen(item->string), &rect, RGBA_MERGE(item->rgb, m_alpha * m_dwOptionAlpha / 100));
						}
					}
...
void cListDialog::AddItem( const char * str, DWORD color, int line, DWORD dwData, DWORD dwSortData, BOOL bColorfulMegaphone, DWORD dwBackColor )
{
	ITEM* pItem = new ITEM;
	strcpy( pItem->string, str);
	pItem->bColorful = bColorfulMegaphone;
	pItem->rgb = color;
	pItem->backColor = dwBackColor;
	pItem->line = line;
	pItem->nFontIdx = 0 ;

	// 090827 ShinJS --- Item Data�� ������ ���� Data ���� �߰�
	pItem->dwData = dwData;
