...
void CCharacterCalcManager::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_CHAR_DETECT_NOTIFY:						Char_Detect_Notify( pMsg );				break;
		// 090507 LUJ, ùù˚/ùùù ùùùùùù? ùùùùù HPùù ùùùùù?ùùù ùùùù
...
void CCharacterCalcManager::Char_Mana_Ack( void* pMsg )
{
	...
	if(HERO)
	{
		// 100223 ShinJS --- ùùùù ùùùùùù ù?ùùù ùùùù ùùùù ùùùùùù ùù?ùùùùùù ùùùù.
...
void CCharacterCalcManager::Char_Exppoint_Ack( void* pMsg )
{
    ...
    // 080930 LYW --- CharacterCalcManager : ?ùùùù ùùùùù ?ùù ùùùù HERO ùùùùùù NULLù?ù? ù?ù.
...
void CCharacterCalcManager::Char_LevelupPoint_Notify( void* pMsg )
{
	...
	// ùùùù ùùùù msg->dwData1
	if(HERO)
	{
		if(msg->wData != 0)
		{				
			GAMEIN->GetCharacterDialog()->SetPointLeveling(TRUE, msg->wData);		// ùùùù? ùùù
		}
		else
 		{
	...
	// 080506 LYW --- CharacterCalcManager : ùùùùùùùù ùùùù ùù tutorialùù ¸?ù?ù ù?ù ?ùù.
...
int CCharacterCalcManager::Notify_SecondJob( BYTE jobType, BYTE byIndex )
{
	...
	case 3 :
		...
		break ;
	// 090930 ONS ù?ùùùùù ùùùùù?ùùù ù?ù
	case 4 :
		{
			nMsgNum = 1133 ;
		}
	...
	case 2 :
		{
			...
			// 080709 LYW --- CharacterCalcManager : ùù?ùùùùùù ùùùù ù?ù.
...
// desc_hseos_ùùù?ùùù01
// S ùùù?ùùù ù?ù added by hseos 2007.05.23	2008.01.11:CPlayer->CHero
...
// E ùùù?ùùù ù?ù added by hseos 2007.05.23	2008.01.11:CPlayer->CHero
...
void CCharacterCalcManager::Initialize( CHero* hero )
{
	...
	for(POSTYPE part = TP_WEAR_START ; part < TP_WEAR_END ; ++part )
	{
		...
		// 091211 LUJ, ù?ùùù ùùùùù?ùùù ùùùù ùù? ù??ù ùùù? ùùùù ùùù ùùùùùùùù ù?¥ù
		//			ù?ùùù ùùùù ù˜ùùùù?ù, ùùù? ùù?ù??ù ùùùù ùùùùùù ùùùù ùùùùù?ù
		//			ù?ù?ùˇùùù Ûùùù?ù
		...
		// ùù? ùùùùùù ù?ù ù?ùùù ùùùù, ùùùù ùùùùùù ùù? ùù?ùù?ùù ùùùù ùùùùùù ùùùùùù ùùùùù?ùùù
		{
...
void CCharacterCalcManager::CalcCharStats( CHero* hero )
{
	...
	// ùùùùùù ùùùùù??ù ùùùù ?ùùùù ùù? ùùùù
...
void CCharacterCalcManager::AddSetItemStats( CHero* hero, const SetScript& script, int setItemSize )
{
		...
		++it, ++level )
	{
		...
		// ù?ùùù ù?ùùùùù
		{
			...
			// ùùùù! ùù ùùùùùùùùùù ùù?ùù ùù?ùù ù?ùùù ù?ù ùùùù
			for(
...
void CCharacterCalcManager::AddPlayerJobSkill(CHero* pHero)
{
	...
	for( WORD grade = CurGrade; grade > 0; grade-- )
	{
		...
		// ùùùù ùùùùùù ù¥ù ùùùù ù?ùùùùù ù?¥ù.
		wJobIdx = 0 ;											
...
// 080320 LUJ, ù?ù ùùÊ? ùù?ùù ù?ùùù ùùùùùù
	...
	if( script && option.mEnchant.mLevel )
	{
			...
			++it )
		{
			...
			case ReinforceScript::eTypeMagicDefence:
				...
				// 080319 LUJ, ù?ù ùùÊ? ùù?ùù ù?ùùù ùù ùùùù ù?ùùù
...
// 080320 LUJ, ù?ù ùùÊ? ùù?ùù ù?ùùù ùùùù ùùùùùù
