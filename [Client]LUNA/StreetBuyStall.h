...
{
	// Chat_Msg와 연동
	eSE_ERROR=502,				//502	기본 에러
	eSE_BUYER_NOT_SPACE = 1216,	//1216
	eSE_BUYER_NOT_MONEY = 1217,		//1217	
	eSE_RARE,					//1219
	eSE_OPTION,					//1220  강화 아이템은 판매 할수 없습니다.

	eSE_NOT_ALLOW = 1223,		//1223
	eSE_NOT_SELECT = 1431,		// 판매할 아이템을 선택해 주세요.
	eSE_NOT_HAVE = 1430,				//1227
	eSE_NOT_EXIST = 1438,				//1226 해당 아이템이 없습니다.
	
	//091112 pdy 구매 노점에 물품 판매시 단축창에 등록되어 있을때 메시지가 잘못 나오는 버그 수정  
	eSE_QUICK = 437,			//437
	eSE_WEARED = 359,			//359
};
...
public:
	...
	void RegistMoney();
	void RegistMoney( POSTYPE pos, DWORDEX dwMoney );
	void RegistTitle( char* title, BOOL bSucess );
	void EditTitle( char* title );
	
public:
	void GetTitle( char* pStrTitle );
	BUY_REG_INFO GetBuyRegInfo(POSTYPE pos) const { return m_RegInfoArray[pos]; };
	...
	DWORDEX GetItemMoney( POSTYPE pos ) { return m_MoneyArray[pos]; }
	...
	void ChangeItemStatus( POSTYPE pos, WORD volume, DWORDEX money);
	...
	static void OnDivideItem( LONG iId, void* p, DWORDEX param1, void * vData1, void * vData2 );
	static void OnDivideItemCancel( LONG iId, void* p, DWORDEX param1, void * vData1, void * vData2 );
	static void OnFakeRegistItem( LONG iId, void* p, DWORDEX param1, void * vData1, void * vData2 );
	static void OnRegistItemCancel( LONG iId, void* p, DWORDEX param1, void * vData1, void * vData2 );
