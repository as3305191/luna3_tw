...
/*
		// 100613 ONS Developer�� GM�� Move, Item&Skill, Hide, Mob��ɸ� �̿밡��.
		BOOL bIsEnable = TRUE;
...
BOOL CGMToolManager::OnGMMoveCommand( int nMethod )
{
	...
	else if( nMethod == 1 )
	{
		...
		if( strncmp( HERO->GetObjectName(), buf, MAX_NAME_LENGTH+1 ) == 0  )	//�ڱ� �ڽ�
			return FALSE;
...
BOOL CGMToolManager::OnUserMoveCommand( int nMethod )
{
	...
	{
		if( strncmp( HERO->GetObjectName(), UserName, MAX_NAME_LENGTH ) == 0  )	//�ڱ� �ڽ�
			return FALSE;
...
BOOL CGMToolManager::OnItemCommand()
{
	...
	{
		char lpstrMoney[MAX_PATH] = { 0, };
		GetDlgItemText(m_hWndSub[eMenu_Item], IDC_GMITEM_EDT_MONEY, lpstrMoney, MAX_PATH);
		char maxmoney[MAX_PATH] = { 0 };
		MONEYTYPE dwMoney = 0;
		sprintf(maxmoney, "%I64u", MAX_INVENTORY_MONEY);
		if (strlen(lpstrMoney) > strlen(maxmoney))
			dwMoney = MAX_INVENTORY_MONEY;
		else
			dwMoney = _atoi64(lpstrMoney);
		if (dwMoney > MAX_INVENTORY_MONEY)
			dwMoney = MAX_INVENTORY_MONEY;
...
BOOL CGMToolManager::OnEventCommand()
{
	...
	// 071210 LYW --- GMToolManager : �̺�Ʈ ����ġ�� ����� �츰��.
	// ����ġ �ְ� 30��� ������ �д�.
	if( nRatio > 3000 ) 
	{
...
//#ifdef TAIWAN_LOCAL	//�߱��� �̺�Ʈ Ȱ��ȭ �ȵ�
				...
				case IDC_GMMOVE_BTN_USER:
					{
						...
						//IDC_GMMOVE_BTN_NAME�� üũ�Ǿ� �ְ�
						//IDC_GMMOVE_EDT_NAME�� enable�̸� disable��
						if( SendMessage( GetDlgItem( hWnd, IDC_GMMOVE_BTN_NAME ), BM_GETCHECK, 0, 0 ) == BST_CHECKED )
						{
...
void CGMToolManager::AddChatMsg( char* pName, char* pMsg, int nKind )
{
	...
	if( nKind == 1 )	//�ߴ�.
	{
		...
		if( m_nNumNow > 10 )
		{
			SendMessage( hWndComboID, CB_DELETESTRING, 10, 0 );	//���� �Ʒ��� �����
			--m_nNumNow;
		}
	...
	else if( nKind == 2 )	//�޾Ҵ�.
	{
		...
		if( m_nNumWait > 10 )
		{
			SendMessage( hWndComboID, CB_DELETESTRING, nCount-1, 0 );	//���� �Ʒ��� �����
			--m_nNumWait;
		}
	}
	else if( nKind == 3 )	//����
