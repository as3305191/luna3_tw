...
{
	// 080602 LYW --- FamilyDialog : ����ġ ó�� (__int32)���� (__int64)�� ���� ó��.
...
void CFishingManager::Fishing_FPChange_Ack(void* pMsg)
{
	...
	m_dwFishPoint += pmsg->dweData;

	CHATMGR->AddMsg( CTC_EXPENDEDITEM, CHATMGR->GetChatMsg( 1539 ), pmsg->dweData );
	...
	char szValue[50];
	sprintf( szValue, "%I64u", m_dwFishPoint);
	GAMEIN->GetFishingDlg()->SetPointText(szValue);
	GAMEIN->GetFishingPointDlg()->SetPointText(szValue);
