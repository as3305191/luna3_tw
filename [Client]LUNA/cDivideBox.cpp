...
// 070504 LYW --- cDivideBox : Include resource manager.
...
void cDivideBox::CreateDivideBox( LONG x, LONG y, LONG ID, cbDivideFUNC cbFc1, cbDivideFUNC cbFc2, void * vData1, void* vData2, char* strTitle )
{
	...
	m_pSpin->SetValue( _atoi64(DEFAULT_VOLUME_TEXT) );
...
{
	// 070504 LYW --- cDivideBox : Modified message number.
	/*
	if( nKind == 0 )
	{
	...
	*/
	...
	if( nKind == 0 )
	{
		m_pOKBtn->SetText( RESRCMGR->GetMsg(11), RGB_HALF(225,225,225), 0xffffffff, 0xffffffff );
		m_pCancelBtn->SetText( RESRCMGR->GetMsg(12), RGB_HALF(225,225,225), 0xffffffff, 0xffffffff );
	}
	else if( nKind == 1 )
	{
		m_pOKBtn->SetText( RESRCMGR->GetMsg(11), RGB_HALF(225,225,225), 0xffffffff, 0xffffffff );
...
DWORD cDivideBox::ActionEvent( CMouse * mouseInfo )
{
	...
	if( weBtn  & WE_BTNCLICK )
	{
		if( m_cbDivideFunc )
		{
			if( GetID() == DBOX_INPUT_MONEY )
			{
				if( _atoi64(RemoveComma(m_pSpin->GetEditText())) > MAX_INVENTORY_MONEY )
				{
					m_cbDivideFunc(	m_ID, this, MAX_INVENTORY_MONEY, m_vData1, m_vData2 );
				}
				else
...
void cDivideBox::SetValue( MONEYTYPE val )
{
	m_pSpin->SetValue(val);
}

MONEYTYPE cDivideBox::GetValue()
{	
	if(m_pSpin)
...
void cDivideBox::SetMaxValue( MONEYTYPE val )
{
	m_pSpin->SetMax( val );
}

void cDivideBox::SetMinValue( MONEYTYPE val )
{
	m_pSpin->SetMin( val );
}