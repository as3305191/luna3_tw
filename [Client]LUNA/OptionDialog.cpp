...
void COptionDialog::UpdateData( BOOL bSave )
{
	...
	if( bSave )
	{
		...
		m_GameOption.nWASDMode		= ((cPushupButton*)pDlg->GetWindowForID( OTI_PB_WASDMODE ))->IsPushed();
		// 100322 ONS ����Ʈ�˸��� �ڵ���� ���� üũ �߰�
		m_GameOption.bAutoQuestNotice		= ((cCheckBox*)pDlg->GetWindowForID( OTI_CB_AUTOQUESTNOTICE ))->IsChecked();

		m_GameOption.bWingsMoveMode		= ((cCheckBox*)pDlg->GetWindowForID( OTI_CB_MOVE_WINGS ))->IsChecked();
		...
		if(!((cCheckBox*)pDlg->GetWindowForID( OTI_HIDEPARTS_ALL ))->IsChecked())
		{
			...
			if(((cCheckBox*)pDlg->GetWindowForID( OTI_HIDEPARTS_COSTUME_WING ))->IsChecked())
				m_GameOption.dwHideParts |= eOPT_HIDEPARTS_COSTUME_WING;
		}
		else
		{
			m_GameOption.dwHideParts |= eOPT_HIDEPARTS_EQUIP_BAND;
	...
	else	//���� ������ �ͼ� â ���� (�⺻������ â�� ��)
	{
		...
		// ---------------------------------------------
		if( !m_GameOption.nMacroMode && !m_GameOption.nWASDMode ) {
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_CHATMODE ))->SetPush( TRUE );
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_MACROMODE ))->SetPush( FALSE );
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_WASDMODE ))->SetPush( FALSE );
		}
		else if( m_GameOption.nMacroMode && !m_GameOption.nWASDMode ){
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_CHATMODE ))->SetPush( FALSE );
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_MACROMODE ))->SetPush( TRUE );
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_WASDMODE ))->SetPush( FALSE );
		}
		else if( !m_GameOption.nMacroMode && m_GameOption.nWASDMode ){
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_CHATMODE ))->SetPush( FALSE );
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_MACROMODE ))->SetPush( FALSE );
			((cPushupButton*)pDlg->GetWindowForID( OTI_PB_WASDMODE ))->SetPush( TRUE );
		}
		// 100322 ONS ����Ʈ�˸��� �ڵ���� ���� üũ �߰�
		((cCheckBox*)pDlg->GetWindowForID( OTI_CB_AUTOQUESTNOTICE ))->SetChecked( m_GameOption.bAutoQuestNotice );

		((cCheckBox*)pDlg->GetWindowForID( OTI_CB_MOVE_WINGS ))->SetChecked( m_GameOption.bWingsMoveMode );
		...
		if(m_GameOption.dwHideParts == eOPT_HIDEPARTS_NONE)
		{
			...
			((cCheckBox*)pDlg->GetWindowForID( OTI_HIDEPARTS_COSTUME_WING ))->SetChecked( FALSE );
		}
		...
		else
		{
			...
			if(m_GameOption.dwHideParts & eOPT_HIDEPARTS_COSTUME_WING)
				((cCheckBox*)pDlg->GetWindowForID( OTI_HIDEPARTS_COSTUME_WING ))->SetChecked( TRUE );
		}
...
void COptionDialog::OnActionEvent( LONG lId, void* p, DWORD we )
{
	...
	if( we & WE_PUSHUP )		//���� ��ư ó��
	{
		if( lId == OTI_PB_CHATMODE || lId == OTI_PB_MACROMODE || lId == OTI_PB_WASDMODE )
	...
	if( we & WE_PUSHDOWN )		//
	{
		if( lId == OTI_PB_CHATMODE )
		{
			...
			((cPushupButton*)((cDialog*)GetTabSheet( 0 ))->GetWindowForID( OTI_PB_WASDMODE ))->SetPush( FALSE );
		}
	...
	if( we & WE_CHECKED )
	{
		...
		else if(lId == OTI_HIDEPARTS_ALL)
		{
			...
			((cCheckBox*)((cDialog*)GetTabSheet( 2 ))->GetWindowForID( OTI_HIDEPARTS_COSTUME_WING ))->SetChecked(TRUE);
		...
		else if(lId == OTI_HIDEPARTS_COSTUME_WING)
		{
			((cCheckBox*)((cDialog*)GetTabSheet( 2 ))->GetWindowForID( OTI_HIDEPARTS_COSTUME_WING ))->SetChecked(TRUE);
	...
	else if( we & WE_NOTCHECKED )
	{
		...
		else if(lId == OTI_HIDEPARTS_ALL)
		{
			...
			((cCheckBox*)((cDialog*)GetTabSheet( 2 ))->GetWindowForID( OTI_HIDEPARTS_COSTUME_WING ))->SetChecked(FALSE);
